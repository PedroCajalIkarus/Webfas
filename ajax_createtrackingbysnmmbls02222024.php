...
union 
select 4 as typestruct, v_string, wo_serialnumber,v_integer, CONCAT('so_',v_string) as v_string from orders_sn_attributes as orders_attributes inner join orders_sn on orders_sn.idorders = orders_attributes.idorders and  orders_sn.wo_serialnumber = orders_attributes.sn  where wo_serialnumber = '".$vsn."' and orders_attributes.idorders = ".$idorder_so." and idattribute_orders = 21 and  typeregister in( 'SO','WO'  )
and not exists 
(
...
union 
select 4 as typestruct, v_string, wo_serialnumber,v_integer, CONCAT('so_',v_string) as v_string from orders_sn_attributes as orders_attributes inner join orders_sn on orders_sn.idorders = orders_attributes.idorders and  orders_sn.wo_serialnumber = orders_attributes.sn  where wo_serialnumber = '".$vsn."' and orders_attributes.idorders in ( ".$idorder_so.",".$idorder_wo.") and idattribute_orders = 21 and  typeregister in( 'SO','WO'  )
and not exists 
(
	select 2 as typestruct, v_string, wo_serialnumber,v_integer,  v_string from orders_sn_attributes as orders_attributes inner join orders_sn on orders_sn.idorders = orders_attributes.idorders and  orders_sn.wo_serialnumber = orders_attributes.sn  where wo_serialnumber = '".$vsn."' and orders_attributes.idorders = ".$idorder_so." and idattribute_orders = 22 and v_string like '%-%' and LENGTH( split_part(v_string, '-', 1) ) =4 
...
//echo $sqltieneso;
   ...
   //   echo $rowsteps['v_string']."--->".substr( $rowsteps['v_string'], 0, 2)."<br>";
      if (   substr( $rowsteps['v_string'], 0, 2)=="wo"  ||  substr( $rowsteps['v_string'], 0, 4)=="AS01" ||  substr( $rowsteps['v_string'], 0, 4)=="AP01" ||  substr( $rowsteps['v_string'], 0, 4)=="SA01" )
      {
        $have_wo='Y' ;
      }
      if (   substr( $rowsteps['v_string'], 0, 2)=="so" )
      {
        $have_so='Y';
      }
  ...
  echo "*<br>---have_wo:".$have_wo."--controlo_wo:".$controlo_wo."--have_so:".$have_so."--controlo_so:".$controlo_so."--v_new_structure_routing:  ".$v_new_structure_routing."--*";
    ...
    {
        echo "<b><p style='color:red'>ERROR, SAP routing not found.</p></b><br>";
...
nuevoref = nuevoref.replace("trackingorderssaproutingmm", "trackingorders");
...
<?php
  ...
  echo  "<a href='trackingorders.php?isdo=".$idorders."&typeisdo=".$dondeinicio."&encont=".$vsn."'>&nbsp;</a><br><br>";
    ...
    {
      echo "<br>STEPS:".$row_autotest['v_string'];
      $tengofuncionecreada=0;
       $nvostepsrouting = explode("-", $row_autotest['v_string']);
     ...
     ///  so_FINALCAL
               if  ( $row_autotest['v_string']  =="so_UPGRADELIC"  )
                {
                $tengofuncionecreada=1;
                $vvnombre_a_mostrar_en_dvi ='Order Detail :: '.$ref_so." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                crear_steps_so_UPGRADELIC($row_autotest['v_string'] ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);
                
                }

                if  ( $row_autotest['v_string']  =="so_FINALCAL"  )
                {
                $tengofuncionecreada=1;
                $haveSOasing="Y";
                $vvnombre_a_mostrar_en_dvi ='Order Detail :: SO:'.$ref_so." - SN: ".$rowbuscawo['wo_serialnumber'];       
           //   echo "----"."wo_".$row_autotest['v_string']."<--->".$row_autotest['wo_serialnumber']."<--->idorder_so".$idorder_so."<--->ref_so".$ref_so."<--->";
           ...
           //     echo "-a ver".                    $model_ciu_wo;   
                 crear_steps_so_FINALCAL($row_autotest['v_string'] ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step, $row_autotest['v_integer'] );
                
                }
               
                if  ( $row_autotest['v_string']  =="so_ENG-CAL"  )
                {
                $tengofuncionecreada=1;
                $vvnombre_a_mostrar_en_dvi ='Order Detail :: SO:'.$ref_so." - SN: ".$rowbuscawo['wo_serialnumber'];     
                                        
               crear_steps_crear_steps_so_ENGCAL($row_autotest['v_string'] ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);
                
                }
                if  ( $row_autotest['v_string']  =="so_BURNING"   )
                    {
                    $tengofuncionecreada=1;
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: sO:'.$ref_so." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                    crear_steps_so_BURNING($row_autotest['v_string'] ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);                       
                    
                    }

                    if ($row_autotest['v_string']   =="wo_MODULE")
                    {
                      $tengofuncionecreada=1;
                      ///   echo "aca".$row_autotest['v_string'];   
                      $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_wo." - SN: ".$rowbuscawo['wo_serialnumber'];   
                  ////     echo              "*******************".$vvnombre_a_mostrar_en_dvi."*******************";               
                       crear_steps_wo_MODULE($row_autotest['v_string'] ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);                       
                      }

                    if  ( $row_autotest['v_string']   =="wo_ENG-CAL" || $nvostepsrouting[0] == "EC01"  )
                    {
                    $tengofuncionecreada=1;
                    ///   echo "aca".$row_autotest['v_string'];   
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_wo." - SN: ".$rowbuscawo['wo_serialnumber'];   
                ...
                //    echo "----"."wo_".$idorder_wo."<--->".$ref_wo,$model_ciu_wo."<--->".$ref_wo."<--->". $enabled_attachfile_step; 
                       ...
                       crear_steps_wo_ENG($row_autotest['v_string'] ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);                       
                    }

                    ///crear_steps_wo_BURNING.
                    if  ( $row_autotest['v_string'] =="wo_BURNING" || $nvostepsrouting[0] == "BU01"  )
                    {
                    $tengofuncionecreada=1;
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_wo." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                        crear_steps_wo_BURNING($row_autotest['v_string'] ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);                       
                    }
                
                    if  ( $row_autotest['v_string']  =="wo_2ND-ASSY"  )
                    {
                       echo "idorder_wo->".$idorder_wo."ref_wo->".$ref_wo."model_ciu_wo->".$model_ciu_wo."idorder_so->".$idorder_so."ref_so->".$ref_so."model_ciu_so->".$model_ciu_so."->".$vvnombre_a_mostrar_en_dvi."->". $enabled_attachfile_step."->". $row_autotest['v_integer'];
                       $tengofuncionecreada=1;
                       $vvnombre_a_mostrar_en_dvi ='Order Detail :: '.$ref_wo." - SN: ".$rowbuscawo['wo_serialnumber'];
                    //    echo "<br>**************<br>";
                    //    echo "----"."wo_".$row_autotest['v_string']."<--->".$row_autotest['wo_serialnumber']."<--->".$row_autotest['idorders']."<--->".$vvnombre_a_mostrar_en_dvi."<--->".$ref_wo,$model_ciu_wo."<--->".$ref_wo."<--->". $enabled_attachfile_step; 
                    
                        crear_steps_woinfo_ASSY("wo_2ND-ASSY" ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);
                       
                    }

                    if  (  $row_autotest['v_string']  =="wo_ASSY" || $nvostepsrouting[0] == "SA01"  | $nvostepsrouting[0] == "AS01" | $nvostepsrouting[0] == "AP01"  | $nvostepsrouting[0] == "RWKP" )
                    {
                    $tengofuncionecreada=1;
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: '.$ref_wo." - SN: ".$rowbuscawo['wo_serialnumber'];
                 //    echo "<br>**************<br>";
                 //    echo "----"."wo_".$row_autotest['v_string']."<--->".$row_autotest['wo_serialnumber']."<--->".$row_autotest['idorders']."<--->".$vvnombre_a_mostrar_en_dvi."<--->".$ref_wo,$model_ciu_wo."<--->".$ref_wo."<--->". $enabled_attachfile_step; 
                     ...
                     crear_steps_woinfo_ASSY("wo_ASSY" ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);
                    }

                    if  (   $nvostepsrouting[0] == "FC01"  )
                    {
                    $tengofuncionecreada=1;
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: '.$ref_wo." - SN: ".$rowbuscawo['wo_serialnumber'];
                 //    echo "<br>**************<br>";
                 //    echo "----"."wo_".$row_autotest['v_string']."<--->".$row_autotest['wo_serialnumber']."<--->".$row_autotest['idorders']."<--->".$vvnombre_a_mostrar_en_dvi."<--->".$ref_wo,$model_ciu_wo."<--->".$ref_wo."<--->". $enabled_attachfile_step; 
                     ...
                     crear_steps_woinfo_ULTEST("wo_ULTEST" ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);
                    }
                    ...
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: '.$ref_wo." - SN: ".$rowbuscawo['wo_serialnumber'];
                     ...
                     crear_steps_wo_BBUMMS("wo_BBUMMS" ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);
                    }
                    ...
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: '.$ref_wo." - SN: ".$rowbuscawo['wo_serialnumber'];
                     ...
                     crear_steps_wo_BBUMMSRF("wo_BBUMMSRF" ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);
                    }

                    if  ( $row_autotest['v_string']  =="so_ASSY"  )
                    {
                    $tengofuncionecreada=1;
                    $haveSOasing="Y";
              ///         echo "sssssssssssssssssssssssssssssssssssss".$ref_so;
               //    echo "----"."wo_".$row_autotest['v_string']."<--->".$row_autotest['wo_serialnumber']."<--->idorder_so".$idorder_so."<--->ref_so".$ref_so."<--->".$ref_wo,$model_ciu_wo."<--->".$ref_wo."<--->". $enabled_attachfile_step; 
                 
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: '.$ref_so." - SN: ".$rowbuscawo['wo_serialnumber']; 
                    crear_steps_soinfo_ASSY($row_autotest['v_string'] ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);                             
                        
                   
                    }

                    if  ( $row_autotest['v_string']  =="wo_RWKASSY"  )
                    {
                    $tengofuncionecreada=1;
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_wo." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                    crear_steps_woinfo_ASSY($row_autotest['v_string'] ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);
                    
                    }
                    ///Probando aqui wo_2ND-ASSY
                    if   ($row_autotest['v_string']  =="so_2ND-ASSY"   )
                    {
                    $tengofuncionecreada=1;
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: SO:'.$ref_so." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                    ...
                    if   ($row_autotest['v_string']  =="so_INFOSO"  )
                    {
                    $tengofuncionecreada=1;
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: SO:'.$ref_so." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                
                      crear_steps_soinfo_only($row_autotest['v_string'] ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);
                    }
                

                    if  ($row_autotest['v_string']  =="wo_PRECHECK" || $row_autotest['v_string']  =="wo_RWKPRECHECK"  || $nvostepsrouting[0] == "PC01" || $nvostepsrouting[0] == "RWKP" )
                    {
                      $name_steps_prechcl =$row_autotest['v_string'];
                    ...
                    $tengofuncionecreada=1;
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_wo." - SN: ".$rowbuscawo['wo_serialnumber'];       
                    crear_steps_woinfo_PRECHECK( $name_steps_prechcl ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);                       
                 
                    }
                    if  ($row_autotest['v_string']  =="so_PRECHECK"    )
                    {
                    $tengofuncionecreada=1;
                    $haveSOasing="Y";
                ...
                //      echo $ref_so;
                    ...
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: SO:'.$ref_so." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                    crear_steps_soinfo_FINALINSPEC($row_autotest['v_string'] ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_so,$model_ciu_so,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);                       
               
                  }
                    if  ($row_autotest['v_string']  =="wo_A.BURN" || $nvostepsrouting[0] == "AB01"   )
                    {
                    $tengofuncionecreada=1;
                    $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_wo." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                    
                    crear_steps_wo_AFTERBURNING($row_autotest['v_string'] ,$row_autotest['wo_serialnumber'], $idorder_wo,$ref_wo,$model_ciu_wo,$idorder_so,$ref_so,$model_ciu_so ,$vvnombre_a_mostrar_en_dvi, $enabled_attachfile_step);                       
                    }
                    

         if (  $tengofuncionecreada==0)
         {
          ?>
<div class="stepazulyamarillo active">
...
<?php
  ...
  //////////////
    ...
    /////////////////////////////////////////////////////////////////////////
function crear_steps_so_UPGRADELIC ($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
{
  include("db_conect.php"); 
...
function crear_steps_crear_steps_so_ENGCAL ($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
{
  include("db_conect.php"); 
...
<div class="step  "> <span class="icon"><i class="	fas fa-box"></i> </span> <span
        class="text"><?php echo substr($vv_worcenter,3,12); ?> <br>
...
function crear_steps_so_BURNING ($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
{
  include("db_conect.php"); 
...
function crear_steps_so_FINALCAL ($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile, $nroItemSteps)
{
  include("db_conect.php"); 
...
<div class="step <?php echo  $activo_paso3; ?>">
    ...
    <br><a
        href="<?php echo $namephp;?>?idsndib=<?php echo  $vv_sn; ?>&amp;iduldl=0&amp;idmb=0&idrunaferbur=<?php echo $idruninfowo_ENG;?>"
        ...
        if ($no_idruninfowo_ENGCAL==0)
         {
          $psswdtkkey = substr( md5(microtime()), 2, 8);
           ?>
    <div class="step "> <span class="icon"> <i class="fas fa-box"></i> </span> <span
            class="text"><?php echo $vv_worcenter_show; ?><br>
...
function crear_steps_wo_AFTERBURNING ($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
{
    ////////////////////////inio WO BURNING
  ...
  $psswdtkkey = substr( md5(microtime()), 2, 8);
    ...
    inner join ( select reference, v_integer as idscript from fas_outcome_integral 
              ...
              and idfasoutcomecat = 0 and idtype= 12 and v_integer in( 33,38,52 )
               ) as lossub
    on lossub.reference = fas_outcome_integral.reference
    where fas_outcome_integral.idfasoutcomecat = 0 and 
    fas_outcome_integral.idtype= 13    
       ...
       if ( $ciuisenterprice =='Y')
        ...
        <a href="#"
            onclick="show_info('finalchkenterpriseremoto','<?php echo $vv_sn; ?>','<?php echo $v_idp; ?>','<?php echo $nombre_a_mostrar_en_dvi_finalchk; ?>','<?php echo   $idruninfoAfertburnung; ?>',0)"><span
                ...
                class="text"><b><?php echo substr($vv_worcenter,3,12); ?> <br>
                    <?php
                 }
                 if ( $ciuismaster =="Y")
                 {
                                ...
                                <a href="#"
                                    onclick="show_info('finalchk','<?php echo $vv_sn; ?>','<?php echo $v_idp; ?>','<?php echo $nombre_a_mostrar_en_dvi_finalchk; ?>','<?php echo $idruninfoAfertburnung; ?>',0)"><span
                                        class="icon"><i class="fas fa-box"></i> </span> <span class="text"><b>
                                            <?php echo substr($vv_worcenter,3,12); ?> <br>
                                            <?php
             }?>
...
function crear_steps_soinfo_FINALINSPEC($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
{ 
  include("db_conect.php"); 
...
/////////////////////////////////BBU MMS ////////
function crear_steps_wo_BBUMMS ($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
 { 
 ...
 ?>
        <div class="step <?php echo  $activo_paso3; ?>">
            <a href="#"
                ...
                <span class="icon">
                    <i class="fas fa-box"></i> </span> <span class="text"><b>MMS [BBU] <br>
...
/////////////////////////////////BBU MMS RF ////////
function crear_steps_wo_BBUMMSRF ($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
 { 
 ...
 ?>
        <div class="step <?php echo  $activo_paso3; ?>">
            <a href="#"
                onclick="show_info('<?php echo $name_js_report;?>','<?php echo $vv_sn; ?>','<?php echo $idorderwo; ?>','<?php echo $nombre_a_mostrar_en_dvi_calib; ?>','<?php echo $idruninfowo_ENG; ?>',0)">
                <span class="icon">
                    <i class="fas fa-box"></i> </span> <span class="text"><b>MMS [RF] <br>
                        <?php echo $activo_paso3_totalpass; ?></span></a></b>
...
/////////////////////////////////FIN BBU  MMS RF ////////
   ...
   /////////////////////////////////UL TEST//
   function crear_steps_woinfo_ULTEST ($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
 { 
     include("db_conect.php"); 
        ...
        <div class="stepverde  active">
            ...
            <div class="step  ">
             ...
             //    echo $vv_worcenter."--".$vv_sn."--".$vv_idp."--".$vvnombre_a_mostrar_en_dvi."--".$vv_soworam."--".$vv_modelciu."--<br>";
                ...
                <a href="#"
                    onclick="show_info('Precheckultest','<?php echo $vv_sn; ?>','<?php echo  $wo_info; ?>','<?php echo $modelciuwo; ?>','Quality Calibration Precheck','<?php echo $vv_worcenter; ?>')">
                    <span class="icon"> <i class="fas fa-tasks"></i> </span>
                    <span class="text"> <b><?php echo  substr($vv_worcenter,3,12);?> <br>SN [<?php echo $vv_sn; ?>]
                            <br></b>
   ...
   function crear_steps_woinfo_PRECHECK ($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
 { 
     include("db_conect.php"); 
            ...
            <div class="stepverde  active">
             ...
             //    echo $vv_worcenter."--".$vv_sn."--".$vv_idp."--".$vvnombre_a_mostrar_en_dvi."--".$vv_soworam."--".$vv_modelciu."--<br>";
                 ?>
                    ...
                    <a href="#"
                        onclick="show_info('Precheck','<?php echo $vv_sn; ?>','<?php echo  $wo_info; ?>','<?php echo $modelciuwo; ?>','Quality Calibration Precheck','<?php echo $vv_worcenter; ?>')">
                        <span class="icon"> <i class="fas fa-tasks"></i> </span>
                        <span class="text"> <b><?php echo  substr($vv_worcenter,3,12);?> <br>SN [<?php echo $vv_sn; ?>]
                                <br></b>
  ...
  if ( $v_enable_attachfile=="Y") {
    ...
    ?><br>
                        ...
                        <?php } ?>
                        <?php
                                                  if( $_SESSION["g"] =='develop' || 'Productionadmin' ==  $_SESSION["g"]|| 'Quality' ==  $_SESSION["g"])
                                                  {
                                                    ?>
     ...
     ///  	echo "test:".$sqldetectchkeopicki;
         ...
         ?>
                        ...
                        <a href="#"
                            ...
                            <span class="icon"> <i class="fas fa-tasks"></i> </span>
                            <span class="text"> <b><?php echo substr($vv_worcenter,3,12);  ?> <br>SN
...
function crear_steps_wo_BURNING($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
{
 /////**************************************************** 
...
?>
                    <div class="step <?php echo  $activo_paso3; ?>"> <a href="#"
                            onclick="show_info('<?php echo $name_js_report;?>','<?php echo $vv_sn; ?>','<?php echo $idorderwo; ?>','<?php echo $nombre_a_mostrar_en_dvi_calib; ?>','<?php echo $idruninfowo_ENG; ?>',0)">
                            <span class="icon"> <i class="fas fa-box"></i> </span> <span class="text"><b>BURNING <br>
...
///////// fin
 ...
 ///////////////////////////////////
 function crear_steps_wo_MODULE ($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
 {
  /////**************************************************** 
  /////**** receive $refSowo = IDRUNIFNO !!!!!!! */
  include("db_conect.php"); 
 ...
 ?>
                    <div class="step <?php echo  $activo_paso3; ?>">
                        <a href="#"
                            onclick="show_info('<?php echo $name_js_report;?>','<?php echo $vv_sn; ?>','<?php echo $idorderwo; ?>','<?php echo $nombre_a_mostrar_en_dvi_calib; ?>','<?php echo $idruninfowo_ENG; ?>',0)">
                            <span class="icon">
                                <i class="fas fa-box"></i> </span> <span class="text"><b><?php echo  $msjlbl; ?> <br>
                                    <?php echo $activo_paso3_totalpass; ?></span></a></b>
 ...
 { ?><br>
  ...
  ///////////////////////////////////
function crear_steps_wo_ENG ($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
{
 /////**************************************************** 
 /////**** receive $refSowo = IDRUNIFNO !!!!!!! */
 include("db_conect.php"); 
...
?>
                        <div class="step <?php echo  $activo_paso3; ?>">
                            <a href="#"
                                onclick="show_info('<?php echo $name_js_report;?>','<?php echo $vv_sn; ?>','<?php echo $idorderwo; ?>','<?php echo $nombre_a_mostrar_en_dvi_calib; ?>','<?php echo $idruninfowo_ENG; ?>',0)">
                                <span class="icon">
                                    <i class="fas fa-box"></i> </span> <span class="text"><b>ENG-CAL <br>
                                        <?php echo $activo_paso3_totalpass; ?></span></a></b>
...
function crear_steps_soinfo_ASSY($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
  { 
    include("db_conect.php"); 
 // echo "HOLA".$vv_worcenter."-wo".$vvv_idwo_nom."-SO".$vvv_idso_nom."-modelWO".$vvv_idwo_ciu ."-modelso".$vvv_idso_ciu;
    $vv_worcenter_show = substr(trim($vv_worcenter),3,10);
     ...
     $datapicking = $connect->query($sqldetectchkeopicki)->fetchAll();
         ...
         ?>
                                ...
                                <a href="#"
                                    ...
                                    <span class="icon"> <i class="fas fa-tasks"></i> </span>
                                    <span class="text"> <b><?php echo  $vv_worcenter_show;?> <br>SN
                                            [<?php echo $vv_sn; ?>] <br></b>
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
function crear_steps_woinfo_ASSY($vv_worcenter,$vv_sn, $vvv_idwo,$vvv_idwo_nom,$vvv_idwo_ciu , $vvv_idso,$vvv_idso_nom,$vvv_idso_ciu, $vvnombre_a_mostrar_en_dvi, $v_enable_attachfile)
  { 
    include("db_conect.php"); 
