    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="row">
          ...
          $sql = $connect->prepare( "select idruninfo,  loginfo from runinfodb  where dateserver < '2019-01-01' and dateserver > '2018-05-01'  and idruninfo > 1200 limit 40 ");
      ...
      foreach ($resultado_station as $row) 
      {												
      //  echo "<br>*-".$rowstation["loginfo"];	
        $porciones = explode("\n", $row['loginfo']);
        $v_cantlineas=0;
        $vlimitlinea = 1000;
        foreach ($porciones as &$valor) 
        {
          ...
          /*SO = 20190119SO
11:28:21.543	UNIT CIU  = DH737-R-2FO-004
11:28:21.553	DB CIU  = DB-033
11:28:21.563	Unit Serial  = 19046243FU
11:28:21.563	DB Serial  = 00004054SP
11:28:21.593	Date: 20190821_112821
11:28:21.593	User: emaresma
*/
           //***************************************
            $pos =strpos($valor, 'SO =');          
					  if ($pos ==0  )	{  } else { $v_so  =substr( $valor,$pos +4); }
            //***************************************
            //***************************************
            $posa =strpos($valor, 'DB CIU  =');          
					  if ($posa ==0  )	{  } else {  $v_dibciu  =substr( $valor,$posa+10 ); }
            //***************************************
             //***************************************
             $posb =strpos($valor, 'UNIT CIU  =');          
             if ($posb ==0  )	{  } else {   $v_sn = substr( $valor,$posb+11 ); }
             //***************************************
              //***************************************
              $posc =strpos($valor, 'DB Serial  =');          
              if ($posc ==0  )	{  } else {   $v_sndib  =substr( $valor,$posc +12); }
              //***************************************
                //***************************************
                $posd =strpos($valor, 'User');          
                if ($posd ==0  )	{  } else {    $v_user =substr( $valor,$posd+5,50 );  }
            
                //***************************************
                if ( $v_cantlineas > $vlimitlinea )
                {
                  break;
                }
                $v_cantlineas=  $v_cantlineas+ 1;
        }
     
      
        echo "<br>Vamos a insertar: <br>Runinfo*".$row['idruninfo']."*<br>v_user*".$v_user."*";
        echo "<br>DB Serial*".$v_sndib."*<br>UNIT CIU*". $v_sn."*" ;
        echo "<br>DB CIU*".$v_dibciu."*<br>SO *". $v_so."* <hr>" ;
      }
