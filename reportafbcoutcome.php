...
inner join idband
on idband.idband = orders_sn_specs.idband	
	where typedata = 'UNIT' and  (so_soft_external like '%WO') and  wo_serialnumber =  '".$vparam_vnrounitsn."'");
...
from (
	 ...
	 ARRAY_AGG ( fas_outcome_integralmn.v_double order by fas_step.titlestep ) as lastolerancias,
	 ARRAY_AGG ( fas_outcome_integralmno.v_double order by fas_step.titlestep ) as lasrefimd,
	 ARRAY_AGG ( fas_outcome_integralm.v_double order by fas_step.titlestep ) as lasrefe
	 from fnt_select_allfas_routines_process_sn_maxrev_byscript('".$vparam_vnrounitsn."',33) as maxrev_sn
	 ...
	 on  fas_outcome_integral.reference = maxrev_sn.iduniqueop and
		 ...
		 left join fas_outcome_integral as fas_outcome_integralmn
		on  fas_outcome_integralmn.reference = maxrev_sn.iduniqueop and
			fas_outcome_integralmn.idfasoutcomecat =2 and
			fas_outcome_integralmn.idtype =1  

		left join fas_outcome_integral as fas_outcome_integralm
		on  fas_outcome_integralm.reference = maxrev_sn.iduniqueop and
			fas_outcome_integralm.idfasoutcomecat =2 and
			fas_outcome_integralm.idtype =32

			left join fas_outcome_integral as fas_outcome_integralmno
		on  fas_outcome_integralmno.reference = maxrev_sn.iduniqueop and
		fas_outcome_integralmno.idfasoutcomecat =2 and
		fas_outcome_integralmno.idtype =7
								 ...
								 <?php
									...
									echo "".$rowresult['description']." ".$label_ULDL_amostrar."</td>";
										...
										for ($ym=0; $ym<count($cabezadetabla2);  $ym++) 
										{
											...
											<td style='font-size: 10pt;text-align: left;border-top: 1px solid black;border-right: 1px solid black;border-left: 1px solid black; border-bottom: 1px solid black'>
											<?php
											 if ($cabezdatosref[$ym] =="NULL")
											 {
												// echo "es null";
												 if ($ym == 1)
												 {
												//	echo "IS IMD3";
													echo " <= ".	$cabezdatosimdymas[$ym]." dBm ";
												 }
												 if ($ym == 4)
												 {
												//	echo "IS NF";
													echo " <= ".	$cabezdatosimdymas[$ym]." dB  ";
												 }
												 if ($ym == 5)
												 {
												//	echo "IS SPI";
											 ...
											 {
												echo $cabezdatosref[$ym]." dB ± ".$cabezdatostol[$ym]."dB ";
											 }
