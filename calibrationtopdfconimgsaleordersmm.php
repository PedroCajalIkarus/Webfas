...
<?php } ?></td>
<td style="text-align: right"><strong><?php echo "2022-04-01  13:03:06 ";
...
inner join runinfodb 	on runinfodb.idruninfodb = fas_finalcheckref_result.idruninfo  where   fas_finalcheckref_result.idruninfo  = 10987119419 and  fas_finalcheckref_result.idruninfo in(SELECT  MAX(idrununfo) as idruninfo from fas_tree_measure where fas_tree_measure.iduniquebranch like '002%' and unitsn = '".$vparam_vnrounitsn."' )
union
select 3, fas_finalcheckref_result.* , runinfodb.userruninfo 
from fas_finalcheck_references_result as fas_finalcheckref_result inner join runinfodb 	on runinfodb.idruninfodb = fas_finalcheckref_result.idruninfo  where   fas_finalcheckref_result.idruninfo  = 10987119419 and  modelciu not in(select distinct  modelciu from products where  idproduct in (select distinct idproduct from products_attributes where v_boolean= true and idattribute = 0)) and  fas_finalcheckref_result.idruninfo in( select idrununfo from fas_tree_measure where fas_tree_measure.iduniquebranch like '002%' and unitsn = '".$vparam_vnrounitsn."'
and datetime in(
	SELECT  MAX(datetime) as idruninfo from fas_tree_measure
...
inner join runinfodb on runinfodb.idruninfodb = fas_finalcheckref_result.idruninfo 	  
where   fas_finalcheckref_result.unitsn = '".$vparam_vnrounitsn."' and fas_finalcheckref_result.idruninfo  = 10987119419 and 
...
<td style="text-align: left"><?php if ( $ismaster=="N") { ?> <?php echo floor($v_dl_gain_pwr); ?> (dBm) <?php } ?> </td>
<th style="text-align: left"><?php if ( $ciuisremote=="N") { ?> UL Max Pwr Out: <?php } ?></th>
<td style="text-align: left"><?php if ( $ciuisremote=="N") { ?>  <?php echo floor($v_ul_gain_pwr); ?> (dBm) <?php } ?> </td>
</tr>
<tr>
						  ...
						  foreach ($datacabez as $rowheaders) 
						  {
							  ...
							  if ($vparam_band_temp9=="")
							{
								...
								if ( $vtemp_idruninfo <= $rowheaders['idruninfo'])
								{
									 ...
									 $vtemp_idruninfo=10987119419;
									 $vparam_idruninfo = 10987119419;
								}
								else
								{
									$vparam_idruninfo = 0;
