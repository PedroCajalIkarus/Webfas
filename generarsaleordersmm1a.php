...
if( $_SESSION["l"]  =="NN")
{
	...
	if (isset($_POST['txtlistcustomers']))
	{
		...
		$connect->beginTransaction();
			...
			*/
					$sentenciach = $connect->prepare("insert into orders_sn_specs  SELECT idorders, idrev,   (0+ row_number() OVER ()) , 0, 'UNIT', 0, 0, 0, 0, 0, 0, v_unitdlstart, v_unitdlstop, v_unitulstart, v_unitulstop, '', idband, v_ulgain, v_dlgain, v_ulmaxpwr, v_dlmaxpwr
					...
					on objectband.idproduct = orders.idproduct AND
					  ...
					  inner join (
						...
						from objectband where idproduct in (select idproduct from orders_sn where idorders = :idorders  )
					...
					WHERE orders_sn_specs.idband  IN(8,1,6,7) and
					orders_sn_specs.idorders =:idorders AND typedata='DPX' AND idnroserie = 0 and orders_sn_specs.idrev = :idrev
 ...
 VALUES ((select COALESCE(max(idpetition),0) + 1 from fas_petitions_server), 2, ".$iduuff.", ".$iduu.", ".$v_id_station.",'04D068', now(), 0, null, '".$parajson."', null, null, null);";
					...
					{
						////// Enviamos mail de aviso a diego..
						...
						$mail->addAddress('marco.moretti@fiplex.com', 'DEVELOPMENT');
							  ...
							  $mail->Subject = "WEBFAS::SO preloaded: ".$txtsonumberlu." - ".$v_esciu ;
							  $mail->Body = "Attention, missing loading parameters to process it.  * Order:  ".$txtsonumberlu." - ".$v_esciu." - ".	$v_temocus ;
							//Definimos AltBody por si el destinatario del correo no admite email con formato html 
							  $mail->AltBody = "WEBFAS::SO preloaded: ".$txtsonumberlu." - ".$v_esciu ;
					...
					/// fin petitions_server
						...
						where iduniquebranchsonprod like '000100010038%' and orders.idorders = :v_idorders ");
		...
		<section class="col-lg-12 connectedSortable ui-sortable">
			...
			<!-- aca form -->
			      ...
			      <form action="changeuserfasdata.php" method="post" class="form-horizontal" name="frmpass" id="frmpass">
							 ...
							 <form  action="generarsaleordersmm1.php" method="post" class="form-horizontal" id="myform" name="myform">
				  <!-- NUEVO RENGLON FORM  -->
    ...
    <div class="col">	UL Channels :: copy and paste the channels here
  				...
  				<button name="btnaddchannels3" id="btnaddchannels3" type="button" class="btn btn-sm btn-outline-primary btn-flat" onclick="importar_nowl()">Import now </button>
					...
					<div class="col-sm-10">
						...
						<button type="button" class="btn btn-primary btn-block" id="btnchangepmm" name="btnchangepmm">Create New PO</button>
					</div>
				  </div>
		...
		htmlmejoradodpx += '<tr> <th width="100px"  class="table-dark text-center"  style="width: 10px">Band</th> <th  class="table-primary text-center" >DPX Low Start </th> <th  class="table-primary text-center" >DPX Low Stop </th><th  class="table-info text-center" >DPX High Start </th><th  class="table-info text-center" >DPX High Stop </th> </tr> </thead> <tbody>';
			...
			//	console.log(tabla_dpx[0]);
				...
				//	console.log('tabla_dpx[i].dpxlowstartcustom:'+tabla_dpx[i].dpxlowstartcustom);
					...
					htmlmejoradodpx += '<tr><th width="100px" scope="row">'+tabla_dpx[i].Band+'   &nbsp;&nbsp; <a href="#" onclick="borrar_array_dpx('+i+')" title="Delete the DPX"><i class="fas fa-times-circle"></i></a>   </th>';
					htmlmejoradodpx += '<td  width="150px"  contenteditable="true" class=" text-center '+v_temp_dpxlowstartcustomclass+'  id'+i+'#dpxlowstartcustom'+'" id="id'+i+'#dpxlowstartcustom" onkeypress="return soloNumeros(event)"  onblur="modif_table_arraydpx('+"'id"+i+'#dpxlowstartcustom'+"'"+','+i+')" >'+v_temp_dpxlowstartcustom+' </td>';
					htmlmejoradodpx +='	<td width="150px"   contenteditable="true" class=" text-center '+v_temp_dpxlowstopcustomomclass+'  id'+i+'#dpxlowstopcustom'+'" id="id'+i+'#dpxlowstopcustom" onkeypress="return soloNumeros(event)"  onblur="modif_table_arraydpx('+"'id"+i+'#dpxlowstopcustom'+"'"+','+i+')" >'+v_temp_dpxlowstopcustom+' </td>';
					htmlmejoradodpx += '<td  width="150px" contenteditable="true" class=" text-center '+v_temp_dpxhighstartcustomclass+'  id'+i+'#dpxhighstartcustom'+'" id="id'+i+'#dpxhighstartcustom" onkeypress="return soloNumeros(event)"  onblur="modif_table_arraydpx('+"'id"+i+'#dpxhighstartcustom'+"'"+','+i+')" >'+v_temp_dpxhighstartcustom+' </td>';
					htmlmejoradodpx +=	'<td width="150px"  contenteditable="true" class=" text-center '+v_temp_dpxhighstopcustomclass+'  id'+i+'#dpxhighstopcustom'+'" id="id'+i+'#dpxhighstopcustom" onkeypress="return soloNumeros(event)"  onblur="modif_table_arraydpx('+"'id"+i+'#dpxhighstopcustom'+"'"+','+i+')" >'+v_temp_dpxhighstopcustom+' </td>';
