...
$elwhere="";
	...
	{
		$elmaxruninfoafiltrar = ($_REQUEST['idb'] * 100000000000);
		$elmaxruninfoafiltrarhasta = (($_REQUEST['idb']+1)* 100000000000);
	}
	if ($elmaxruninfoafiltrar =="")
	{
		...
		if ($_SESSION["i"]!=1)
		{
			$elmaxruninfoafiltrar = ( $_SESSION["i"] * 100000000000);
		    $elmaxruninfoafiltrarhasta = (($_SESSION["i"]+1)* 100000000000);
		}
	///	echo "aaaaaaaaaaaaaaaaaaa".$elmaxruninfoafiltrar;
	}
	...
	if ( $param_search=="fiplexlog2")
	{
		...
		{
			$elwhere=" where ";
		    $wherebusiness = " userruninfo  in ('c.user','t.user','spin_test11','spin_test10') and idruninfodb >=".$elmaxruninfoafiltrar." and  idruninfodb <=".$elmaxruninfoafiltrarhasta;
			$wherebusiness = " userruninfo  in ('c.user','t.user','spin_test11','spin_test10') ";
	...
	$sql = $connect->prepare("select count(*) as cc from  runinfodb ");
    $sql->execute();
    $resultado = $sql->fetchAll();
	 foreach ($resultado as $row) {
		...
		if 	($_SESSION["g"] == "develop"  ) 
		{
			if(strlen($elwhere)<6)
			{
				$elwhere =	$elwhere." where dateserver >'2018-11-07'";
			//	$elwhere =	$elwhere." where dateserver >'2020-01-01'";
			}
			else
			{
				$elwhere =$elwhere." and  dateserver >'2018-11-07'";
			//	$elwhere =$elwhere." and  dateserver >'2020-01-01'";
		...
		else
		{
			if(strlen($elwhere)<6)
			{
				$elwhere =	$elwhere." where dateserver >'2021-01-01'";
			}
			else
			{
				$elwhere =$elwhere." and  dateserver >'2021-01-01'";
	...
	FROM runinfodb ".$elwhere." and script not in ('SAP Connector') and userruninfo  not in ('fas_sap','usersapconect')   ORDER BY fechamm desc,  hr DESC, hrcorta desc  LIMIT ".$rowid." OFFSET ".$rowperpage);
