       ...
       <hr>
            ...
            <div class="card " id="artSO" name="artSO">
                  ...
                  if (filter_var($sanitized_n, FILTER_SANITIZE_STRING)) {
                    ...
                    /* $sql = $connect->prepare("select  distinct modelciu,  orders.quantity,orders_sn.so_soft_external, wo_serialnumber, fas_tree_measure.dibsn
                    from orders
                    inner join orders_sn 
                    on orders_sn.idorders = orders.idorders
                    inner join products
                    on products.idproduct = orders.idproduct
                    left join fas_tree_measure
                    on fas_tree_measure.unitsn = orders_sn.wo_serialnumber 
                    where orders.idorders = ".$v_idp."
                    and idnroserie >0");
                    ////version mejorada
                    */
                //    echo  $filtrar_xsn;
                    if (  $filtrar_xsn =="FU")
                    {
                       ...
                       $sql = $connect->prepare("select distinct  1 as orr, orders.idorders  ,
                    ...
                    if (  $filtrar_xsn =="WO")
                    {
                          $sql = $connect->prepare("select distinct  orders.idorders  ,
                          orders.processfasserver::int as processfasserver,  orders_sn_SOassociada.idorders as idordersso,  
                         coalesce(productsso.modelciu,'') as tienesomodelciuso, coalesce(orders_sn_SOassociada.so_soft_external,'') as tienesoasociada, 
                         products.modelciu,  orders.quantity,orders_sn.so_soft_external, orders_sn.wo_serialnumber, tienecalibracion.dibsn
                         ,        tienecalibracion.totalpass::int as tienecalibration_totalpass, tienecalibracion.calibrationscript::int as tienecalibration_calibrationscript,
           ...
           $sql->execute();
           $resultadobbwo = $sql->fetchAll();							
            $encontroalgoenlos3select = "N";
           $cant_sn=0;
           $cant_sn_assign=0;
           $ciutemp="";
           $sotemp="";
           foreach ($resultadobbwo as $rowbuscawo) 
           {
           ...
           $activo_paso1_processfasserver ="";
             ...
             ?>  <div class="track">
                    ...
                    <a href="#" onclick="show_info('orderinfo','<?php echo $rowbuscawo['wo_serialnumber']; ?>','<?php echo $v_idp; ?>','<?php echo $nombre_a_mostrar_en_dvi; ?>',0,0)">
                        ...
                        <span class="text text-left"><b>WO: [<?php echo $rowbuscawo['so_soft_external']; ?>]<br>CIU: [<?php echo $rowbuscawo['modelciu']; ?>]</b> 
              ...
              //         echo "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
                ...
                //	echo "test:".$sqldetect;
                  ...
                  /////**************************************************** 
                  /////**************************************************** 
               //   echo  $ciuisenterprice."---------------------".$ciuisremote;
                   if(  $ciuisenterprice=="Y" &&  $ciuisremote=="Y")
                   {
                   }
                   else
                   { 
                     ///Ocultado hasta terminar el pickeo
                      /*
                      if ($activo_paso2 <>"")
                      {
                        ?>
                        <div class="step <?php echo  $activo_paso2; ?>"> <a href="#" onclick="show_info('acceptance','<?php echo $rowbuscawo['dibsn']; ?>','<?php echo $v_idp; ?>','<?php echo $nombre_a_mostrar_en_dvi_acept; ?>',0,0)"><span class="icon"> <i class="fas fa-sliders-h"></i> </span> <span class="text"><b>Digital Board Acceptance <br>SN [<?php echo $rowbuscawo['dibsn']; ?>] <br> <?php echo $activo_paso2_totalpass; ?> </b></span></a> </div>             
                        <?php
                      }
                      else
                      {
                        ?>
                        <div class="step <?php echo  $activo_paso2; ?>"> <span class="icon"> <i class="fas fa-sliders-h"></i> </span> <span class="text">Digital Board Acceptance <br>SN [] <br></span> </div>             
                        <?php
                      }
                      */
                  } 
...
<?php
        ...
        ///  echo "a ver".$nombre_ciu_amostrar;
          ...
          if ( $encontroalgoenlos3select == "N")
            {
              $sqlSOsinnada = $connect->prepare("select  distinct orders.quantity, orders_sn.*, modelciu from orders inner join orders_sn  on orders_sn.idorders = orders.idorders 
              inner join products
              on products.idproduct = orders.idproduct   where orders.idorders  = ".$v_idp."    and idnroserie =0   ");

              $sqlSOsinnada->execute();
              $resulSosinnada = $sqlSOsinnada->fetchAll();	
              echo "avccc";
              foreach ($resulSosinnada as $rowssinnada)
              {

                for ($cantSnagenerar=0; $cantSnagenerar< $rowssinnada['quantity']; $cantSnagenerar= $cantSnagenerar+ 1){

                  ?>
                  <div class="track">
                    <div class="step "> 
                                       <span class="icon"> <i class="fa fa-check"></i> </span> <span class="text text-left"><b>WO: [ ]<br>CIU: [ ]</b> <br><b> SN Generated: [ ]</b>
                    </div>
                    <div class="step "> <span class="icon"> <i class="fas fa-sliders-h"></i> </span> <span class="text">Digital Board Acceptance <br>SN [] <br></span> 
                    </div>   
                     <div class="step ">  <span class="icon">  <i class="fa fa-box"></i> </span> <span class="text">Calibration<br></span> </div>
                    <div class="step "> <span class="icon"><i class="fa fa-box"></i> </span> <span class="text">After Burning Check<br> </span></div>
                  
                    <a href="#" onclick="show_info('orderinfo','<?php echo $rowssinnada['wo_serialnumber']; ?>','<?php echo $rowssinnada['idorders']; ?>','<?php echo $nombre_a_mostrar_en_dvi_paraSO; ?>',0,0)">
                    <div class="step active ">
                      <span class="icon"> <i class="fa fa-check"></i> </span>
                       <span class="text"><b>SO:<?php echo $rowssinnada['so_soft_external']; ?> <br>CIU : <?php echo  $rowssinnada['modelciu']; ?><br>SN Assign :  <?php echo $rowssinnada['wo_serialnumber']; ?></b></span>
                    </a> 
                    
                    <a href="#" onclick="Assign_sn(<?php echo $rowssinnada['idorders']; ?>,'<?php   echo $rowssinnada['so_soft_external'];  ?>')"> <i class='fas fa-wrench'></i> Assign SN </a>
                    </div> 
                     <div class="step  "> <span class="icon"> <i class="fa fa-box"></i> </span> <span class="text">Final Check</span> </div>
                  </div>
                  <br>  <br>
		...
		<section class="col-lg-12 connectedSortable ui-sortable">
				...
				<div class="card">
                   <div class="card-header">
                       <h3 class="card-title" id="titudetalle" name="titudetalle"> <i class="fa fa-fw fa-list-alt"></i> </h3>
...
<div class="chart tab-pane pre-scrollablemarco card-body active " id="divdetalles" name="divdetalles" style="position: relative; ">
