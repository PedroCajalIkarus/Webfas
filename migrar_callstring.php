  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Migrador CALL o Function</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Migrador CALL o Function</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    ...
    <section class="content">
      ...
      $linea = " select tmp_equivpnciu.honpartnumbnew, tmp_equivpnciu.fiplexpartnumb,
      ppp.idproduct as idprodhon,   pppm.idproduct as idprodfiplex, 
      concat('0010',substring(pppm.iduniquebranchsonprod,5,50))as newbnramchs
      from tmp_equivpnciu
      inner join fnt_select_allproducts_maxrev_bysap() as ppp
      on ppp.modelciu = tmp_equivpnciu.honpartnumbnew
      inner join fnt_select_allproducts_maxrev_bysap() as pppm
      on pppm.modelciu = tmp_equivpnciu.fiplexpartnumb 
      where tmp_equivpnciu.honpartnumbnew not in('BDA-HC3-W2','BDA-PSL8-W2')
           ";
    //  echo $linea."<br>";
     $datacabez = $connect->query($linea)->fetchAll();
   
       
  
      // $return_arr = array();
      foreach ($datacabez as $row) 
       {
         echo  $mmm."<hr><br>-Replicando desde ".$row['fiplexpartnumb']." - hasta:".$row['honpartnumbnew']."<br>";
         $mmm =  $mmm + 1;


         /// UPATEO A la nueva empresa y BRACNH

         ///REPLICAMOS.
         $sqlupdar="  update products set active = 'Y', idbusiness = 10,  iduniquebranchsonprod = '".$row['newbnramchs']."'          where idproduct =  ".$row['idprodhon'];
         echo "<br>".$sqlupdar;
           // $sqlmma = $connect->prepare( $sqlupdar);
              // $sqlmma->execute();

         $modelciu_from = $row['fiplexpartnumb'];
         $modelciu_to = $row['honpartnumbnew'];

         $idprodu_modelciu_from = $row['idprodfiplex'];
         $idprodu_modelciu_to = $row['idprodhon'];
    ...
    ////////////// REPLICAMOS OUT COME
              $query_2recursive=" select replicate_income_integral_allidtype0(0,".$idprodu_modelciu_from.",".$idprodu_modelciu_to.",0) ";
              $dataiscuu = $connect->query($query_2recursive)->fetchAll();		
              echo "<br>".$query_2recursive;
    
              $query_2recursive=" select replicate_income_integral_nft_allidtype0(0,".$idprodu_modelciu_from.",".$idprodu_modelciu_to.",0) ";
              $dataiscuu = $connect->query($query_2recursive)->fetchAll();		
              echo "<br>".$query_2recursive;
