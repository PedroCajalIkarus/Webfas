...
foreach ($datacabeznomband as $rowheadersnomband) 
{
	...
	if ($vparam_band==9)
	{
	  ...
	  $sql = $connect->prepare("select distinct products.idbusiness,  products.modelciu, orders_sn.* , showdpxreport, fas_tree_product.idfastree , COALESCE(orders_sn_specs.ulgain,0) as ulgainnew , COALESCE(orders_sn_specs.dlgain,0) as dlgainnew,
...
where so_soft_external like '%SO' and wo_serialnumber =  '".$vparam_vnrounitsn."'");
...
select 2 as ordrnnn, fas_finalcheckref_result.* , runinfodb.userruninfo from fas_finalcheckref_result inner join runinfodb 	on runinfodb.idruninfodb = fas_finalcheckref_result.idruninfo  where fas_finalcheckref_result.idruninfo in(SELECT  MAX(idrununfo) as idruninfo from fas_tree_measure where fas_tree_measure.iduniquebranch like '002%' and unitsn = '".$vparam_vnrounitsn."' )
union
select 3, fas_finalcheckref_result.* , runinfodb.userruninfo from fas_finalcheckref_result inner join runinfodb 	on runinfodb.idruninfodb = fas_finalcheckref_result.idruninfo  where modelciu not in(select distinct  modelciu from products where  idproduct in (select distinct idproduct from products_attributes where v_boolean= true and idattribute = 0)) and  fas_finalcheckref_result.idruninfo in( select idrununfo from fas_tree_measure where fas_tree_measure.iduniquebranch like '002%' and unitsn = '".$vparam_vnrounitsn."'
and datetime in(
	SELECT  MAX(datetime) as idruninfo from fas_tree_measure
where fas_tree_measure.iduniquebranch like '002%' and unitsn = '".$vparam_vnrounitsn."') )
union 
select 4, fas_finalcheck_references_result.idruninfo, modelciu, unitsn, dibsn, gaintolerance, maxpwrtolerance, imdlimit, nfreference, spuriousreference, fpgafirm, ucfirm, ethernetfirm, fasversion , 
runinfodb.userruninfo from fas_finalcheck_references_result inner join runinfodb 	on runinfodb.idruninfodb = fas_finalcheck_references_result.idruninfo  where modelciu not in(select distinct  modelciu from products where  idproduct in (select distinct idproduct from products_attributes where v_boolean= true and idattribute = 0)) and  fas_finalcheck_references_result.idruninfo in( select idrununfo from fas_tree_measure where fas_tree_measure.iduniquebranch like '002%' and unitsn = '".$vparam_vnrounitsn."'
and datetime in(
	SELECT  MAX(datetime) as idruninfo from fas_tree_measure
...
///echo "ciuisdas".$ciuisdas."-------".$ciuiscentrix;
if(  $ciuisdas =="Y" && 	$ciuiscentrix=="Y")
{
	...
	fas_calibration_result.dibsn	= fas_tree_measure.dibsn and 
	fas_calibration_result.idruninfo =  fas_tree_measure.idrununfo
		where fas_tree_measure.iduniquebranch like '002%' and fas_calibration_result.modelciu  in(select distinct  modelciu from products where  idproduct not in (select distinct idproduct from products_attributes where v_boolean= true and idattribute = 0)) 
		and fas_tree_measure.unitsn = '".$vparam_vnrounitsn."' GROUP BY fas_tree_measure.uldl,fas_tree_measure.band   )
	as losmasejecutados
	on fas_tree_measure.uldl = losmasejecutados.uldl and
	fas_tree_measure.band =  losmasejecutados.band and 
...
else
{
	if ($v_dnd =="WO")
		...
		fas_tree_measure.datetime = losmasejecutados.masfechaidruninfo
		where fas_tree_measure.unitsn = '".$vparam_vnrounitsn."' order by fas_tree_measure.band, fas_tree_measure.uldl	";	
	//	echo "<br>aca el ultio:".$sql;
		}
}

	//echo $v_dnd."<<<<<<<marco:".$sql."<br>";	
...
////aca tenemos un problema..						
	...
	//	echo "<br>a ver".$query_lista."<br>";
			...
			//	echo "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa".$v_ban;
					...
					if(  $ciuisdas =="Y")
					{
						echo "".$label_band_amostrar; ?> <a href="finalchkallband.php?dnd=<?php echo $v_dnd;  ?>&idsndib=<?php echo $vparam_vnrounitsn; ?>&idmb=<?php echo $v_ban ; ?>&iduldl=<?php echo $v_uldl; ?>&idruninfo=<?php echo $vparam_idruninfo; ?>#"> <i class="fas fa-eye"></i> </a></td>
						<?php
					}
					else
					{
						echo "".$label_band_amostrar ." - ".$label_ULDL_amostrar; ?> <a href="finalchkallband.php?dnd=<?php echo $v_dnd;  ?>&idsndib=<?php echo $vparam_vnrounitsn; ?>&idmb=<?php echo $v_ban ; ?>&iduldl=<?php echo $v_uldl; ?>&idruninfo=<?php echo $vparam_idruninfo; ?>#"> <i class="fas fa-eye"></i> </a></td>
					<?php
					}
			  ?></td>
				     <?php
...
<!--5555 NUEVA TABLA RESUMEN -->

<!--5555 fin nueva tabla resumen -->
