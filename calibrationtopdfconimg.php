...
error_reporting(0);
if ($_REQUEST['tracking']=="nomoestramasxahora")
				{
					?>
...
if ($_REQUEST['idrun'] <> "")
{
///----- BUSCAMOS LA FECHA DEL RUN
$sqlfecharun ="select * from fas_outcome_integral where reference = ".$_REQUEST['idrun']." limit 1";
///echo $sqlfecharun;
$datacabeznomband = $connect->query($sqlfecharun)->fetchAll();
$datetimerun = "";
foreach ($datacabeznomband as $row_udfecha) 
{
	$datetimerun = substr($row_udfecha['datetimeref'],0,19);
}
if ($datetimerun == "" )
{
	$datetimerun = date("m/d/y h:m:s") ;
}
...
?>
                        ...
                        <td style="text-align: right"><strong><?php echo $datetimerun; ?></strong></td>
                    </tr>
                </table>
...
union 
select distinct 3, fas_finalcheck_references_result.idruninfo, modelciu, unitsn, dibsn,  gaintolerance, maxpwrtolerance, imdlimit, nfreference, spuriousreference, fpgafirm, ucfirm, ethernetfirm, fasversion , runinfodb.userruninfo 
from fas_finalcheck_references_result 
inner join runinfodb on runinfodb.idruninfodb = fas_finalcheck_references_result.idruninfo 
where modelciu in(select distinct modelciu from products where idproduct in 
				  (select distinct idproduct from products_attributes where v_boolean= true and idattribute = 0)) and 
	fas_finalcheck_references_result.idruninfo in( select idrununfo from fas_tree_measure where fas_tree_measure.iduniquebranch like '002%' and 
			unitsn = '".$vparam_vnrounitsn."')   ";
			
			/*and datetime 
							in( SELECT MAX(datetime) as idruninfo from fas_tree_measure where fas_tree_measure.iduniquebranch like '002%' and unitsn = '".$vparam_vnrounitsn."') ) 
"; */
...
objectband.idband = orders_sn_specs.idband
	where typedata = 'UNIT' and (so_soft_external not like '%SO'  and so_soft_external not like '%RM')  and  wo_serialnumber =  '".$vparam_vnrounitsn."'");
	 /// $sql->bindParam(':vvidsndib', $vparam_vnrounitsn);
    $sql->execute();
    $resultado3 = $sql->fetchAll();
	foreach ($resultado3 as $row2) 
...
inner join idband
on idband.idband = orders_sn_specs.idband	
	where typedata = 'DPX' and (so_soft_external not like '%SO' and  so_soft_external not like '%RM' )  and  wo_serialnumber =  '".$vparam_vnrounitsn."'");
	 /// $sql->bindParam(':vvidsndib', $vparam_vnrounitsn);
    $sql->execute();
    $resultado3 = $sql->fetchAll();
	foreach ($resultado3 as $row2) 
...
////aca tenemos un problema..						
		 ...
		 if ($tienedatosparalatabla>0)
		 {
		  ...
		  ?>
            ...
            <?php
// 29-07-2021 a pedido de agus frenamos aca
exit();
		...
		?>
            <div class="page_break" id="content<?php echo "".$label_band_amostrar ." - ".$label_ULDL_amostrar; ?>"
                ...
                <h3 style="text-decoration: underline;">
                    <?php echo "".$label_band_amostrar ." - ".$label_ULDL_amostrar; ?></h3>
                <table
                    style="width: 100%;font-size: 10pt;text-align: left;border-top: 2px solid black;border-right: 2px solid black;border-left: 2px solid black; border-bottom: 2px solid black;">
                ...
                <table
                    ...
                    <tr>
                        <td style="width:340px;height:19px;">Noise Figure
                            <canvas id="salesChartlevel<?php echo $vvv_sumanombregraf; ?>" width="338px" height="245px"
                                style="height: 245px;width:338px"></canvas>
                        </td>
                        <td style="width:340px;height:190px;">Gain vs PwrIn @796.5MHz
                            <canvas id="graficoabajo1<?php echo $vvv_sumanombregraf; ?>" width="338px" height="245px"
                                style="height: 245px;width:338px"></canvas>
                        </td>
                    </tr>
                </table>
                ...
                <table
                    ...
                    <tr>
                        <td style="width:340px;height:190px;">PwrOut vs PwrIn @796.5MHz
                            <canvas id="graficoabajo2<?php echo $vvv_sumanombregraf; ?>" width="338px" height="245px"
                                style="height: 245px;width:338px"></canvas>
                        </td>

                        <td style="width:340px;height:190px;">PwrIn Dynamic Range
                            <canvas id="graficoabajo3<?php echo $vvv_sumanombregraf; ?>" width="338px" height="245px"
                                style="height: 245px;width:338px"></canvas>
                        </td>
                    </tr>
                </table>
                <table
                    ...
                    <tr>
                        <td style="width:340px;height:190px;">Ch AGC vs PwrIn @796.5MHz
                            <canvas id="graficoabajo4<?php echo $vvv_sumanombregraf; ?>" width="338px" height="245px"
                                style="height: 245px;width:338px"></canvas>
                        </td>

                        <td style="width:340px;height:190px;">BB AGC vs PwrIn @796.5MHz
                            <canvas id="graficoabajo5<?php echo $vvv_sumanombregraf; ?>" width="338px" height="245px"
                                style="height: 245px;width:338px"></canvas>
