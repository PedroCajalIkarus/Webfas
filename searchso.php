...
include("db_conect.php"); 
    ...
    session_start();
        if (isset($_POST['search']) && isset($_POST['date'])) {
            ...
            {
                $sql = $connect->prepare("
                select  distinct iduniquebranchsonprod, products.idproduct ,  pre.idcustomers, namecustomers ,
                    orders_sn.so_soft_external, pre.active,fassrverror, pre.processfasserver, pre.idorders,pre.idrev,
                    products.modelciu ciu, quantity,  pre.date_approved as datestate , max(prestatus.idstate) as idstates,
                    count(distinct orders_sn_asignados.wo_serialnumber) as cantsnasing ,
                    array_agg(coalesce(orders_sn_asignados.wo_serialnumber,'')) as groupxsn,
                    ...
                    min(orders_attributesrabbit.v_boolean::integer) as 	haverabbit  ,
                    products.active as activeprod ,  atrib30.v_string as issoblock,
                ...
                where  ".$filter_date_pedro."  
                (
                     customers.namecustomers LIKE '%".$search."%' or products.modelciu Like '%".$search."%'
                ) and
                ( 
                    (pre.typeregister = 'PO' and pre.active <>'N'   ) 
                    or (pre.typeregister = 'SO' and pre.active <>'N'   ) 
                    or (pre.typeregister = 'UP' and pre.active <>'N'  ) 
                )
                ...
                order by  datestate desc	");
                    ...
                    where  ".$filter_date_pedro."  
                    (
                         customers.namecustomers LIKE '%".$search."%' or  orders_sn.so_soft_external LIKE '".$search."%' or products.modelciu Like '%".$search."%'
                    ) and
                    ( 
                        (pre.typeregister = 'PO' and pre.active <>'N'   ) 
                        or (pre.typeregister = 'SO' and pre.active <>'N'   ) 
                    ...
                    order by  datestate desc	");
                                ...
                                if(empty($resultado)){
                                    ...
                                    echo '<script>                        toastr.options = {
                                        ...
                                        toastr["error"](" ", "SaleOrder not Found");</script>';
                                    exit();
                                } else {
