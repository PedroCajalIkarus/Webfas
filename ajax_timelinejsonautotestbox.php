...
foreach ($dataheadr as $rowhead) 
	...
	$sqlmmresu="
	select * , products.modelciu as modelciu_so
from
(
	select distinct products.modelciu as modelcuipp, fas_routines_product_sn.sn,fas_calibration_result.*
		...
		left join orders_sn 
		on orders_sn.wo_serialnumber = fff.sn  and orders_sn.typeregister  = 'SO'
		left join products
...
$entrotitu=0;
	foreach ($dataresumen as $rowmm) 
	{
		...
		$retureturn_titulo[] = array(
			...
			"modelciu_so" => trim($rowmm['modelciu_so']) ,
