...
<!-- Site wrapper -->
  <!-- Navbar -->
   <p align="right"> <a href="acceptflex.php?idsndib=<?php echo  $vparam_vnrounitsn;?>" target="_blank"><img src="iconopdf.jpg" width="30px"></a>
								 </p>
...
<?php
	...
	$sqlnombremand ="SELECT modelciu from fas_calibration_result 	 WHERE unitsn='".$vparam_vnrounitsn."'";
	
	$datacabeznomband = $connect->query($sqlnombremand)->fetchAll();
	
	foreach ($datacabeznomband as $rowheadersnomband) 
	{
		if ($rowheadersnomband['modelciu'] =="FIP446" || $rowheadersnomband['modelciu'] =="FIP446_REV006" || $rowheadersnomband['modelciu'] =="FIP446_CONF01" )
		{
			$nombreband_0="700 FirstNet";
			$nombreband_1="800";
		}
		if ($rowheadersnomband['modelciu'] =="FIP467" || $rowheadersnomband['modelciu'] =="FIP467_CONF01_T" || $rowheadersnomband['modelciu'] =="FIP467_EU_CONF01_T" || $rowheadersnomband['modelciu'] =="FIP467_REV004_T" || $rowheadersnomband['modelciu'] =="FIP467_REV004_LB_T" || $rowheadersnomband['modelciu'] =="FIP467_REV004_LB"  )
		{
			$nombreband_0="VHF";
			$nombreband_1="UHF";
...
//echo "<br>Cuadro Resumen<br>".$nombreband_0."---".$nombreband_1;
								...
								$vejecucion = 1;
								  foreach ($datacabez as $rowheaders) 
								  {
									...
									//  echo  "HOLA:". $obj_result_json->{'totalpass'};
									  ...
									  {
										$vparam_idruninfo = $_REQUEST['idrun'];
									  }
								//	  $vparam_idruninfo = $obj_result_json->{'idruninfo'};

									  echo "<p><b>IdRunInfo:".$vparam_idruninfo;
									  echo "</b></p>";
...
fas_tree_measure.iduniquebranch like '00E%'  and unitsn = '".$vparam_vnrounitsn."' and idrununfo =".$vparam_idruninfo. "  and uldl = 0 and band = 0
...
maxretry.maxidrev 			=	fas_tree_measure.idrev
										...
										where fas_tree_measure.iduniquebranch = '00E' and fas_tree_measure.unitsn = '".$vparam_vnrounitsn."' and  fas_tree_measure.idrununfo =".$vparam_idruninfo. " order by fas_tree_measure.band, fas_tree_measure.uldl, iduniqueop";
											 ...
											 foreach ($datacabez2 as $rowheaders) 
											  {
												  if ($rowheaders['uldl'] ==0)
													...
													{
														$label_band_amostrar =	$nombreband_0;
													}
													else
													{
														$label_band_amostrar =	$nombreband_1;
													}
													?>
													<tr>
 ...
 $vtemp_idruninfo=0;
	...
	inner join ( 
		...
		fas_tree_measure.unitsn = '".$vparam_vnrounitsn."' GROUP BY uldl,band order by uldl,band
			...
			and fas_tree_measure.idrununfo in (
				select idrununfo from fas_tree_measure where fas_tree_measure.unitsn = '".$vparam_vnrounitsn."'    and datetime in(
						SELECT DISTINCT  MAX(datetime) as masfechaidruninfo 
				   from fas_tree_measure 
				   where fas_tree_measure.iduniquebranch like '00E%' and fas_tree_measure.unitsn = '".$vparam_vnrounitsn."'   
								) 
								)
				 ...
				 $finalchk_gain_freqshow="";
				  ...
				  unset(  $array_finalchk_pwr);
					...
					$array_finalchk_noisefig="";
									...
									{
										$vtemp_idruninfo=$_REQUEST['idrun'];
										$vparam_idruninfo = $_REQUEST['idrun'];
									}
									...
									$cantbandas = $cantbandas + 1 ;
									  $query_lista="SELECT distinct  fas_tree_measure.totalpass,  fas_tree_measure.iduniqueop,  fas_step.description as namebranch,  fas_tree_measure.iduniquebranch, fas_tree_measure.unitsn, fas_tree_measure.dibsn, fas_tree_measure.uldl, fas_tree_measure.band
										...
										inner join (
											...
											where fas_tree_measure.iduniquebranch like '00E%' and uldl = ".$vaparam_uldl."  and band = ".$vparam_band." and unitsn = '".$vparam_vnrounitsn."'  and idrununfo =".$vparam_idruninfo. " 
										...
										where  fas_tree_measure.iduniquebranch like '00E%' and uldl = ".$vaparam_uldl." and  band = ".$vparam_band." and  fas_tree_measure.unitsn = '".$vparam_vnrounitsn."' and  idrununfo =".$vparam_idruninfo. " order by iduniqueop";
											...
											//	  echo "<br>-- **Antes del IF GAIN".$row['iduniquebranch'];
											  ...
											  if (strcmp( $row['iduniquebranch'], "00E041044") === 0)
											  {
												...
												//echo 	$indice_para_plot."Query plots uldl = ".$vaparam_uldl." and  band = ".$vparam_band.$query_lista5;
													...
													$query_lista5=" select fas_singlemeasures.*,  fas_mkrmeasures.pwr as mkrmeasurewr 
													  from fas_singlemeasures
													  ...
													  and fas_mkrmeasures.iduniqueop = fas_singlemeasures.iduniqueop 
