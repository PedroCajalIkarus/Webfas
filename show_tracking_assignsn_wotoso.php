...
<div class="row">
   ...
   where orders.active ='Y' and length(trim(wo_serialnumber)) =0 			
            ...
            foreach ($resultadostock as $rowstock) 
           {
              ...
              ?>
                <tr>
                    <td>
                        <button type="button"
                            onclick="asinngwotoso_sn(<?php echo  $rowstock['idorders']?>,'<?php echo $rowstock['so_soft_external'] ?>',' <?php echo trim($elsn) ?>',<?php echo $soparam; ?>,'<?php echo $v_so_soft_external_wo;?>',<?php echo $rowstock['proxnnroserie'] ?>)"
                            class="btn  btn-outline-primary btn-xs">Assign to <?php echo   $elnomso;?> </button>
                    </td>
                    <td> <?php echo "<b> ".$rowstock['so_soft_external'];  
                      if ( strlen($rowstock['ponumber']) <5)
                      {
                        echo " <span style='color:red'>PO missing... Attention!</span> ";
                      }
                      else
                      {
                        echo " -- PO: ".$rowstock['ponumber'];
                      }
...
?>
    ...
    <div class="col">
        <p name="msjwaitline" id="msjwaitline" align="center"><img src="img/waitazul.gif" width="100px"> Send
            Information ....</p>
    </div>
</div>
