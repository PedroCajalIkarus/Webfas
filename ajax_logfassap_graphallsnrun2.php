...
$V_fechaha = $_REQUEST['pfh'];
						...
						on  fas_outcome_integral2.reference       = losrun.reference and
						 ...
						 order by v_booleanconvert desc , v_string desc
							...
							$query_listagraf= "
							select count(distinct losxmlsap.id_outcomemm) as qquantity, fas_outcome_integral_sap.v_boolean::integer  as v_booleanconvert, 
							substring(fas_outcome_integral_sap2.v_string,0,22) as v_string
														from 
														(
															...
															and   datetimeref between '".$V_fechadesde."' AND  '".$V_fechaha." 23:59:59'
														) as losxmlsap
														  inner join fas_outcome_integral_sap
									  on fas_outcome_integral_sap.reference = losxmlsap.id_outcomemm
									  and  fas_outcome_integral_sap.idfasoutcomecat= 0 and fas_outcome_integral_sap.idtype=13
															left join fas_outcome_integral_sap as fas_outcome_integral_sap2
									  on fas_outcome_integral_sap2.reference = losxmlsap.id_outcomemm
									  and  fas_outcome_integral_sap2.idfasoutcomecat= 17 and fas_outcome_integral_sap2.idtype=23
									  group by fas_outcome_integral_sap.v_boolean, 
							substring(fas_outcome_integral_sap2.v_string,0,22) 
							order by v_booleanconvert desc , v_string desc
					...
					//	echo $query_listagraf;
							...
							foreach ($data2 as $row3) 
							{
								...
								{
									if ($row3['v_string']=="WO-SO Already Created")
									{
										if ($v_nomb_label=="")
										{
