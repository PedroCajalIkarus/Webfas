...
inner join ( select so.idsaleorders , array_agg(coalesce(ciu,'')) as groupxciu, array_agg(coalesce(sn_unit,'')) as groupxsn 
			from saleorders so inner join saleorders_specs so_sp on so.idsaleorders = so_sp.idsaleorders where so.active = 'Y' group by so.idsaleorders 
		   ) as agrupadoresxidsaleorders
on agrupadoresxidsaleorders.idsaleorders = so.idsaleorders 



where so.active = 'Y' and namecustomers is not null and so_soft_external is not null and so_soft_external like '%SO'  and so_soft_external not like '99%'  group by so.idsaleorders,so_soft_external,namecustomers ,
...
where namecustomers is not null and so_soft_external is not null and so_soft_external like '%RM' group by so.idsaleorders,so_soft_external,namecustomers ,
agrupadoresxidsaleorders.groupxciu, agrupadoresxidsaleorders.groupxsn ,so.datesoapproved1 having count(distinct so_sp.ciu) >0 order by so_soft_external desc
) as  list_so
...
case when coalesce(descripcion,'')='' then '-' when descripcion='null' then '-' else descripcion end as  descripcion,
 ...
 ul_stop ,
  ...
  dl_max_pwr,
dl_start,
  dl_stop ,so_sp.idruninfo,so_sp.ifdualband,
case when so_sp.idruninfo = 0 then '-' when so_sp.idruninfo >0 then trim(runinfodb.userruninfo) end as userruninfo ,nameapproved

from saleorders so inner join saleorders_specs so_sp on so.idsaleorders = so_sp.idsaleorders left join runinfodb


on runinfodb.idruninfo = so_sp.idruninfo where sn_unit = '".$v_snunit."' and ciu ='".$v_idciu."' order by so_sp.idband, so_sp.idrev asc	 ";
	  return $valor_devuelto;
	}
...
on maxbanrevciuunit.idcustomers	= so_sp.idcustomers and
		...
		maxbanrevciuunit.idband			= so_sp.idband			
left join runinfodb
on runinfodb.idruninfo = so_sp.idruninfo 
where so_sp.sn_unit = '".$v_snunit."' and so_sp.ciu ='".$v_idciu."' order by so_sp.idband, so_sp.idrev asc	 ";
	  return $valor_devuelto;
	}
	...
	function list_all_customers($fraseabuscar)
	{
			$valor_devuelto= "select * from customers where upper(namecustomers) like '%".strtoupper($fraseabuscar)."%' and  active = 'Y' order by namecustomers";	
				return $valor_devuelto;
	}
	
	function list_all_ponumber($fraseabuscar)
	{
			$valor_devuelto= "select distinct ponumber from orders_sn where typeregister <> 'WO' order by ponumber ";	
				return $valor_devuelto;
	}
	
	function list_all_cuis($fraseabuscar)
	{
			$valor_devuelto= "select * from products where idproduct >399 and upper(modelciu) like '%".strtoupper($fraseabuscar)."%' and  active = 'Y' order by modelciu";	
				return $valor_devuelto;
	}
		...
		function list_show_SN_by_CIU($tipoplaca, $v_BDciu)
	{
		if ($tipoplaca=="ACF")
		{
			$valor_devuelto= "select distinct 'ACF' as typeaccept, sn, ciu, active from acceptacf where ciu='".$v_BDciu."' AND active = 'Y' order  by sn";	
		}
		if ($tipoplaca=="DB")
		{
			$valor_devuelto= "select distinct 'DB' as typeaccept, sn, ciu , active from acceptdib where ciu='".$v_BDciu."' AND active = 'Y' order by sn";	
		}
		if ($tipoplaca=="PA")
		{
			$valor_devuelto= "select distinct 'PA' as typeaccept, sn, ciu , active from acceptpa where ciu='".$v_BDciu."' AND active = 'Y' order by sn";	
		}
...
order by typeaccept, ciu  ";
 ...
 $valor_devuelto="select distinct 'DB' as typeaccept,  acceptdib.ciu, count(distinct sn)  as cc, array_agg(DISTINCT coalesce(sn,'')) as groupxsn,  coalesce(losaceptados.cc_totalpass,0)   as cctp
from acceptdib 
left join 
(	
...
union 
select distinct 'PA' as typeaccept,  acceptpa.ciu, count(distinct sn)  as cc, array_agg(DISTINCT  coalesce(sn,'')) as groupxsn,  coalesce(losaceptados.cc_totalpass,0)   as cctp
from acceptpa 
left join 
(	
...
union
select distinct 'ACF' as typeaccept,  acceptacf.ciu, count(distinct sn)  as cc, array_agg(DISTINCT  coalesce(sn,'')) as groupxsn,  coalesce(losaceptados.cc_totalpass,0)   as cctp
from acceptacf 
left join 
(	
...
order by  typeaccept,  ciu";
	...
	{
		$valor_devuelto="select band,idit, runinfodb.dateinfo, timescript, userruninfo,station,fasver,  totalpass, fws,   sn_module, sn_unit, ciu_unit, ciu_module ,digmodule.idruninfo  as idruninfo 
from digmodule
inner join runinfodb
on runinfodb.idruninfo = digmodule.idruninfo  where sn_unit = '".$v_sn_unit."' and sn_module = '".$v_sn_module."' order by band,idit";
	...
	{
		$valor_devuelto="select band,id_it, runinfodb.dateinfo,timescriptsteps,userruninfo,station,fasver,  totalpass, fws, coalesce(sn_dib,'') as sn_dib , coalesce(sn_palp,'') as sn_palp ,coalesce(sn_pahp,'' ) as sn_pahp, coalesce(sn_unit,'') as sn_unit, ciu_dib, ciu_palp, ciu_pahp, ciu_unit, freq, calibrationfinalcheck.idruninfo as idruninfo,eqbda

from calibrationfinalcheck
inner join runinfodb
on runinfodb.idruninfo = calibrationfinalcheck.idruninfo
...
on runinfodb.idruninfo = calibrationfinalcheck.idruninfo
 where sn_unit = '".$v_sn_unit."' and sn_dib = '".$v_sn_module."' and step = 1 order by band,id_it";
   ...
   function list_all_attributes_wizard()
   {
	   ?>
   ...
   function list_all_branch_wizard()
   {
	   ?>
	...
	function list_all_values_from_table($nombretabla, $nombrecampo)
	{
			$valor_devuelto="select distinct ".$nombrecampo." from ".$nombretabla." order by ".$nombrecampo." asc";
			return $valor_devuelto;
	}
	
	function update_labeling ($vidlabel, $vnamecolumntable, $vdatoupdate)
	{
		$valor_devuelto="update products_labeling set $vnamecolumntable = '$vdatoupdate' where idlabel = ".$vidlabel;
			return $valor_devuelto;
	}
	
		function insert_labeling ($vidlabel, $vnamecolumntable, $vdatoupdate)
	{
		$valor_devuelto="insert into  products_labeling  values (".$vidlabel.",";
			return $valor_devuelto;
	}
	...
	function list_accept_db_data($tipodb, $nameciu, $dbsn)
	{
		if ($tipodb == 3)
		{/// ACF
			$valor_devuelto="select distinct *, COALESCE (fasver,'')  as fasver2 from acceptacf inner join runinfodb on runinfodb.idruninfo = acceptacf.idacceptacf  where sn = '".$dbsn."' and ciu = '".$nameciu."' order by idacceptacf asc ";
		}
		if ($tipodb == 1)
		{/// DIB
			$valor_devuelto="select distinct *, COALESCE (fasver,'')  as fasver2  from acceptdib inner join runinfodb on runinfodb.idruninfo = acceptdib.idacceptdib  where sn = '".$dbsn."' and ciu = '".$nameciu."' order by idacceptdib asc ";
		}
		if ($tipodb == 2)
		{/// PA
			$valor_devuelto="select distinct *, COALESCE (fasver,'')  as fasver2  from acceptpa inner join runinfodb on runinfodb.idruninfo = acceptpa.idacceptpa  where sn = '".$dbsn."' and ciu = '".$nameciu."' order by idacceptpa asc ";
		}
