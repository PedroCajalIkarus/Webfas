...
if( $_SESSION["l"]  =="NN")
{
	...
	if (isset($_POST['txtlistcustomers']))
	{
		...
		$losdatosoorigin = explode("|", $_REQUEST['txtlistcius']);

		$v1 = $losdatosoorigin[0]; /// idtxtlistcius
		$v2 = $_REQUEST['txtlistcius']; //txtlistcius 
		$v3 = $_REQUEST['txtponumber']; //
		$txtsonumberlu = trim($_REQUEST['txtsonumberlu']); //
		$txtso_original = trim($losdatosoorigin[1]); //
		$txtsn_toupgrade = trim($losdatosoorigin[2]); //

		$id_prod_upgrade = $_REQUEST['txttypeupgrade']; //
		...
		$connect->beginTransaction();
			 try {
					//$connect->query($sql);
						...
						$sentencia->bindParam(':quantity', $vqqcant);		
				...
				//		echo "aca 2";
					...
					//	echo "insert into orders_fileattach SELECT idordersfileat,".$vmaxid.", namefileattach, seedtemp FROM public.orders_fileattach_draft where active = 'draft' and seedtemp =".$v_tkkeymarco1;
						...
						$sentencia->bindParam(':wo_serialnumber', $txtsn_toupgrade);
						$sentencia->bindParam(':idruninfo', $vconcero);					
						...
						$sentencia->bindParam(':processday', $vvacionull );		
						$sentencia->bindParam(':processfasserver', $vprocessdas);	
						$sentencia->bindParam(':so_original', $txtso_original);	
						...
						$sentencia->bindParam(':idconfiguration', $vconcero);				
						$sentencia->bindParam(':idrev', $vconcero);	
						$sentencia->bindParam(':idnroserie', $v_sn);						
						$sentencia->bindParam(':so_soft_external', $txtsonumberlu );
						$sentencia->bindParam(':wo_serialnumber', $txtsn_toupgrade);
						$sentencia->bindParam(':idruninfo', $vconcero);					
						$sentencia->bindParam(':ponumber', $v3);					
						...
						$sentencia->execute();
							...
							$vtextaudit=$vtextaudit."!!txtsonumberlu:".$txtsonumberlu."|".$txtsn_toupgrade;
					...
					/////////////////////////////////////////////////////////////////////////////////////
					  ...
					  $query_lista ="INSERT INTO public.orders_attributes(idorders, idattribute_orders, datemodif, v_boolean, v_integer, v_double, v_string, v_date) VALUES (".$vmaxid.", 2,now(),true, ".$id_prod_upgrade.", NULL,'".$txtsn_toupgrade."', NULL);";
		...
		<section class="col-lg-12 connectedSortable ui-sortable">
			...
			<!-- aca form -->
			      ...
			      <form action="changeuserfasdata.php" method="post" class="form-horizontal" name="frmpass" id="frmpass">
							 ...
							 <form  action="generarsaleordersupgrade.php" method="post" class="form-horizontal" id="myform" name="myform">
				  <!-- NUEVO RENGLON FORM  -->
				  ...
				  <div class="form-group row">
					<label for="inputPassword" class="col-sm-2 col-form-label">Type Upgrade:</label>
					<div class="col-sm-4	">
		
					

					<select class="form-control" required  id="txttypeupgrade" name="txttypeupgrade" required>
					<option value=""> -Select-  </option>
						 <?php
						 
						 
						 $sql = $connect->prepare("select * from products_upgrades ");
						 ...
						 foreach ($resultado3 as $row2) 
						  ...
						  ?>
						  <option value="<?php echo  $row2['idprodup']; ?>" <?php echo $autoselect;?>>
						  <?php echo  $row2['upgrcode']." "; ?>
						  </option>
						  <?php
						  }
...
<br><br>
		...
		<div class="form-group row">
					...
					<div class="col-sm-10">
						...
						<div class="col-sm-2">
						<button type="button" class="btn btn-primary btn-block" id="btnchangep" name="btnchangep">Create New Upgrade SN</button>
					</div>
				  </div>
