...
$elwhere="";
    ...
    $resultado = $sql->fetchAll();
	 foreach ($resultado as $row) {
		...
		if 	($_SESSION["g"] == "develop"  ) 
		{
			if(strlen($elwhere)<6)
			{
				$elwhere =	$elwhere." where dateserver >'2021-11-07'";
			//	$elwhere =	$elwhere." where dateserver >'2020-01-01'";
			}
			else
			{
				$elwhere =$elwhere." and  dateserver >'2021-11-07'";
	...
	FROM runinfodb ".$elwhere." and  userruninfo  in ('fas_sap','usersapconect')   ORDER BY fechamm desc,  hr DESC, hrcorta desc  LIMIT ".$rowid." OFFSET ".$rowperpage);

 
 if ($nropagemm ==2)
 {
echo "SELECT distinct fasver,  idruninfodb, to_char(dateserver,'YYYYY/MM/DD HH24:MI:SS') as fechamm,  to_char(dateserver,'YYYY/MM/DD') as fechacorta,to_char(dateserver,'hh')AS hr,to_char(dateserver,'HH24:MI:SS') as hrcorta,
trim(userruninfo) as userruninfo, case when  trim(station)='Null' then 'Uknown' else trim(station) end as station, case when trim(device)='Null' then 'Uknown' else trim(device) end  as device, trim(Script) as Script,
case when to_char(dateserver,'hh')= '01' then 'A'
when to_char(dateserver,'hh')= '02' then 'A'
when to_char(dateserver,'hh')= '03' then 'A'
