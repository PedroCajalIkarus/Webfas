    ...
    dataTable.addRows([
      ...
      <?php
        ...
        where dateserver > CURRENT_DATE 
			---and userruninfo in('mbatista','ffloridia','emaresma')  
		---	and device like '%Flex%'
            ...
            //    echo "aaaaaaaaaaaaaaaa".$row['userruninfo'];
                if ( trim($usu) <> trim($row['userruninfo']))
                {
                   ...
                   ?>
                         ...
                         [ '<?php echo  $ususation; ?>', '', new Date("<?php echo substr($row['dateinfom'], 0, 10); ?>T17:00:00.000"), new Date("<?php echo substr($row['dateinfom'], 0, 10); ?>T17:00:00.100") ],
                   <?php
...
<script type="text/javascript">
  ...
  function drawChart2() {
    ...
    var container2 = document.getElementById('exampledetalle');
    ...
    dataTable2.addRows([
      ...
      <?php
        ...
        $sqlmm2="  select   fas_step.description as namebranch,station,   userruninfo, runinfodb.script , dateinfom,  dateserver ,runinfodb.idruninfo ,  max (fas_times.duration) as totaltime, 
        sum(fas_times_typett.duration) as totalparcial, max(datetimeend) as datetimeend, fas_times_typett.datetimelog
        ...
        where 
	   	runinfodb.idruninfo = 10907022681222
		 
        group by   fas_step.description , station, userruninfo, runinfodb.script , dateinfom,  dateserver, runinfodb.idruninfo, fas_times_typett.datetimelog
        
        order by userruninfo
        ";

        $datalineality = $connect->query($sqlmm2)->fetchAll();
        $usu="";
							foreach ($datalineality as $row) 
							{
            //    echo "aaaaaaaaaaaaaaaa".$row['userruninfo'];
                if ( trim($usu) <> trim($row['idruninfo']))
                {
                   $usu = trim($row['idruninfo']);
                   ?>
                         [ '<?php echo  trim($row['namebranch']); ?>', '', new Date("<?php echo substr($row['dateinfom'], 0, 10); ?>T07:00:00.000"), new Date("<?php echo substr($row['dateinfom'], 0, 10); ?>T07:00:00.100") ],
                         [ '<?php echo  trim($row['namebranch']); ?>', '', new Date("<?php echo substr($row['dateinfom'], 0, 10); ?>T17:00:00.000"), new Date("<?php echo substr($row['dateinfom'], 0, 10); ?>T17:00:00.100") ],
                   <?php
                ...
                else
                {
                  ...
                  ?>
                  [ '<?php echo  $usu; ?>', '<?php echo  trim($row['script'])."-".trim($row['idruninfo'])  ; ?>', new Date("<?php echo $row['dateinfom']; ?>"), new Date("<?php echo $row['datetimelog'];  ?>") ],
                  <?php
                //  $textoamostrar =  $textoamostrar ."usu:".$usu."-Script:".trim($row['script'])." -> FI:".$row['dateinfom']."-FF:".$fechafin."<br>";
                }
    ...
    var options = {
        ...
        colors:['red','green','orange','orange'],
...
<div id="exampledetalle" style="height: 500px;"></div>
