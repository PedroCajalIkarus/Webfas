           ...
           //         echo "<br>mmmmm".$sqlbusco."<br>";
                   ...
                   <?php
                      ...
                      orders.quantity,orders_sn.so_soft_external, orders_sn.wo_serialnumber 
                               ...
                               where  orders.typeregister <>'".$typeisdo."' and orders_sn.so_soft_external = '".$rowbuscawo['so_associed']."' and wo_serialnumber ='". $rowbuscawo['wo_serialnumber']."'"; 
                         
                   

                   
                       ///    echo "<br>Query Principal:".$query_wo."<br><hr>";
              ...
              //      echo  "<hr>".$query_xml."<hr>";
                ...
                */
                         ...
                         for($immt=0; $immt< $qq_components_xml; $immt++)
                         {
                          ...
                          $ejecuto_solucionador_fixer = $ejecuto_solucionador_fixer + 1;
                  
                      //aca poner if q como cargar las variables.
                      if (  $typeisdo == "SO")
                      {
                          $ref_woso = $rowbuscawo['so_soft_external'];
                          $model_ciu = $rowbuscawo['modelciu'];
                      }
                      if (  $typeisdo == "WO")
                      {
                        $ref_woso = $rowbuscawo['so_associed'];


                            $query_woattach2 ="select idorders, modelciu from orders_sn 
                                                inner join products
                                               on products.idproduct = orders_sn.idproduct
                                                where so_soft_external = '". $ref_woso."' and wo_serialnumber ='".$rowbuscawo['wo_serialnumber']."' ";
                        ///   echo "<br>****#######".$query_woattach2."<br>****#######" ;
                            $data_womm = $connect->query($query_woattach2)->fetchAll();	
                           
                            foreach ($data_womm as $rowwommm) 
                            {
                              $v_idp =$rowwommm['idorders'];
                              $model_ciu = $rowwommm['modelciu'];
                     ...
                     // echo "<br>-------".$arraysteps_name[$immt]." -->".$rowbuscawo['so_soft_external']." -- ". $ref_woso."----".$v_idp;



                        ///  so_FINALCAL
                        if  ($arraysteps_name[$immt]  =="so_UPGRADELIC"  )
                        {
                          $tengofuncionecreada=1;
                          $vvnombre_a_mostrar_en_dvi ='Order Detail :: '.$ref_woso." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                     //     crear_steps_so_UPGRADELIC($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$rowbuscawo['modelciu'], $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                          crear_steps_so_UPGRADELIC($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                       
                        }
                        
                        if  ($arraysteps_name[$immt]  =="so_FINALCAL"  )
                        {
                          $tengofuncionecreada=1;
                          $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_woso." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                         // crear_steps_so_FINALCAL($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                          crear_steps_so_FINALCAL($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                       
                        }
                        
                        if  ($arraysteps_name[$immt]  =="so_ENG-CAL"  )
                        {
                          $tengofuncionecreada=1;
                          $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_woso." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                      //    crear_steps_crear_steps_so_ENGCAL($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                          crear_steps_crear_steps_so_ENGCAL($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                       
                        }
                          if  ($arraysteps_name[$immt]  =="so_BURNING"  )
                            {
                              $tengofuncionecreada=1;
                              $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_woso." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                       //       crear_steps_so_BURNING($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                              crear_steps_so_BURNING($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                           
                            }

                            if  ($arraysteps_name[$immt]  =="wo_ENG-CAL"  )
                            {
                              $tengofuncionecreada=1;
                              $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_woso." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                            //  crear_steps_wo_ENG($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                              crear_steps_wo_ENG($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                            }
                       
                            if  ($arraysteps_name[$immt]  =="wo_ASSY"  )
                            {
                              $tengofuncionecreada=1;
                              $vvnombre_a_mostrar_en_dvi ='Order Detail :: '.$ref_woso." - SN: ".$rowbuscawo['wo_serialnumber'];
                      ////        echo "a verrrrrrrrrrrrrrrrrrrrr".$vvnombre_a_mostrar_en_dvi;
                        ///      crear_steps_woinfo_ASSY($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                              crear_steps_woinfo_ASSY($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                            }

                            if  ($arraysteps_name[$immt]  =="so_ASSY"  )
                            {
                              $tengofuncionecreada=1;
                              $vvnombre_a_mostrar_en_dvi ='Order Detail :: '.$ref_woso." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                        ///      crear_steps_woinfo_ASSY($arraysteps_name[$immt] ,$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                            ...
                            if  ($arraysteps_name[$immt]  =="wo_RWKASSY"  )
                            {
                              $tengofuncionecreada=1;
                              $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_woso." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                            ///  crear_steps_woinfo_ASSY($arraysteps_name[$immt],$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                            ...
                            if  ($arraysteps_name[$immt]  =="so_2ND-ASSY"  )
                            {
                              $tengofuncionecreada=1;
                              $vvnombre_a_mostrar_en_dvi ='Order Detail :: SO:'.$ref_woso." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                            //  crear_steps_woinfo_2NDASSY($arraysteps_name[$immt],$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                              crear_steps_woinfo_2NDASSY($arraysteps_name[$immt],$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                            }
                            if  ($arraysteps_name[$immt]  =="wo_PRECHECK" || $arraysteps_name[$immt]  =="wo_RWKPRCHK"  )
                            {
                              $tengofuncionecreada=1;
                              $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_woso." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                           //   crear_steps_woinfo_PRECHECK($arraysteps_name[$immt],$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                              crear_steps_woinfo_PRECHECK($arraysteps_name[$immt],$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                            }
                            if  ($arraysteps_name[$immt]  =="so_PRECHECK"    )
                            {
                              $tengofuncionecreada=1;
                              $vvnombre_a_mostrar_en_dvi ='Order Detail :: SO:'.$ref_woso." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                              
                           //   crear_steps_soinfo_FINALINSPEC($arraysteps_name[$immt],$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                              crear_steps_soinfo_FINALINSPEC($arraysteps_name[$immt],$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                            }
                            if  ($arraysteps_name[$immt]  =="wo_A.BURN"    )
                            {
                              $tengofuncionecreada=1;
                              $vvnombre_a_mostrar_en_dvi ='Order Detail :: WO:'.$ref_woso." - SN: ".$rowbuscawo['wo_serialnumber'];                              
                              
                        //      crear_steps_wo_AFTERBURNING($arraysteps_name[$immt],$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                              crear_steps_wo_AFTERBURNING($arraysteps_name[$immt],$rowbuscawo['wo_serialnumber'], $v_idp,$vvnombre_a_mostrar_en_dvi,$ref_woso,$model_ciu, $arraysteps_name_refso[$immt], $enabled_attachfile_step);
                            }
                          ...
                          <div class="stepazulyamarillo active">
                              ...
                              <span class="icon"> <i class="fa fa-check"></i> </span> <span class="text"><b>SO:<?php echo $arraysteps_name[$immt]; ?>
            ...
            <?php
                  ...
                  if ($cantrefres ==1 )
                  {
                    ...
                    nuevoref = nuevoref.replace("trackingordersmmsap3", "trackingorders");  
...
{ 
  ...
  $psswdtkkey = substr( md5(microtime()), 1, 8);
      /////**************3  Quality Survey Final Check ************************************** 
                                              ...
                                              $sqldetectchkeo3="SELECT   status_sn  FROM public.fas_survey_responses_bysn where idsurvey = 1 and  sn = '".$vv_sn."' and  so = '". $vv_soworam."' and modelciu = '".$vv_modelciu."'
                                           ...
                                           //   echo "test:".$sqldetectchkeo3;
                                              ...
                                              <a href="#" onclick="show_info('Precheck','<?php echo $vv_sn; ?>','<?php echo  $vv_soworam; ?>','<?php echo $vv_modelciu; ?>','Quality Calibration Precheck',0)">
                                                <span class="icon"> <i class="fas fa-tasks"></i> </span> 
                                              <span class="text">  <b>PRECHECK  <br>SN [<?php echo $vv_sn; ?>] <br></b></span>
                                              <?php
                                                $sqldetectchkeo="SELECT   status_sn  FROM public.fas_survey_responses_bysn where  idsurvey = 1 and  sn  = '".$vv_sn."' and  so = '".$vv_soworam."' and modelciu = '".$vv_modelciu."'
                                                order by datetimecheck desc limit 1 ";
...
$activo_paso3_totalpass = "<span class='badge bg-success'>Passed</span><br><a href='#' onclick='popuplogdb(".$idruninfowo_ENG.")'  style='color:#f39323;'>View Log <i class='fas fa-eye'> </i></a>";
            ...
            if( $_if_auto_test_box_calibration == "N"  )
            {
              $no_idruninfowo_ENGCAL=1;
                    ...
                    {
                      ?>
                  <div class="step <?php echo  $activo_paso3; ?>">  <a href="#" onclick="show_info('finalchk','<?php echo $vv_sn; ?>','<?php echo $idorderwo; ?>','<?php echo $nombre_a_mostrar_en_dvi_calib; ?>','<?php echo $idruninfowo_ENG; ?>',0)"> <span class="icon">  <i class="fa fa-box"></i> </span> <span class="text"><b>ENG-CAL <br> <?php echo $activo_paso3_totalpass; ?></span></a></b> </div>
                  <?php
                    }
                    else
                    {
                        ...
                        {
                          ?>
                            <div class="step <?php echo  $activo_paso3; ?>">  <a href="#" onclick="show_info('calibrationentermater','<?php echo $vv_sn; ?>','<?php echo $idorderwo; ?>','<?php echo $nombre_a_mostrar_en_dvi_calib; ?>','<?php echo $idruninfowo_ENG; ?>')"> <span class="icon">  <i class="fa fa-box"></i> </span> <span class="text"><b>ENG-CAL  <br> <?php echo $activo_paso3_totalpass; ?></span></a></b> </div>
                            <?php
                        }
                        else
                        {
                        ?>
                        <div class="step <?php echo  $activo_paso3; ?>">  <a href="#" onclick="show_info('calibration','<?php echo $vv_sn; ?>','<?php echo $idorderwo; ?>','<?php echo $nombre_a_mostrar_en_dvi_calib; ?>','<?php echo $idruninfowo_ENG ; ?>')"> <span class="icon">  <i class="fa fa-box"></i> </span> <span class="text"><b>ENG-CAL <br> <?php echo $activo_paso3_totalpass; ?></span></a></b> </div>
                        <?php
                        }
                    }
            }
            else
            {
              $no_idruninfowo_ENGCAL=1;
                    ...
                    {
                      ?>
                        <div class="step <?php echo  $activo_paso3; ?>">  <a href="#" onclick="show_info('calibrationyburchk','<?php echo $vv_sn; ?>','<?php echo $idorderwo; ?>','<?php echo $nombre_a_mostrar_en_dvi_calib; ?>','<?php echo $idruninfowo_ENG; ?>')"> <span class="icon">  <i class="fa fa-box"></i> </span> <span class="text"><b>ENG-CAL <br> <?php echo $activo_paso3_totalpass; ?></span></a></b>
                        <br> <a href="calibrationtopdfconimg.php?idsndib=<?php echo  $vv_sn ; ?>&amp;iduldl=0&amp;idmb=0" target="_blank">  <i class='fas fa-file-pdf'></i> - View Report</a>
                        <br>
                        <?php if ( $v_enable_attachfile=="Y") { ?>
                 <a href="#" onclick="attachanalogbda(<?php echo $vv_idp; ?>,'<?php echo  $vv_worcenter.'_'.$psswdtkkey;?>','<?php echo $vv_sn; ?>')">
                                                                                                            <i class="fa fa-paperclip" aria-hidden="true"></i>      Attach Files
                    ...
                    {
                      ?>
                      <div class="step <?php echo  $activo_paso3; ?>">  <a href="#" onclick="show_info('calibration','<?php echo $vv_sn ; ?>','<?php echo $idorderwo; ?>','<?php echo $nombre_a_mostrar_en_dvi_calib; ?>','<?php echo $idruninfowo_ENG; ?>')"> <span class="icon">  <i class="fa fa-box"></i> </span> <span class="text"><b>ENG-CAL<br> <?php echo $activo_paso3_totalpass; ?></span></a></b> 
                      <?php if ( $v_enable_attachfile=="Y") { ?>
                 <a href="#" onclick="attachanalogbda(<?php echo $vv_idp; ?>,'<?php echo  $vv_worcenter.'_'.$psswdtkkey;?>','<?php echo $vv_sn; ?>')">
                                                                                                            <i class="fa fa-paperclip" aria-hidden="true"></i>      Attach Files
                                                     </a> <br>
...
// fin// AutoComplete de CUIS version TOP	
   ...
   $("#txtlistcius").change(function(){
            ...
            window.location = 'trackingordersmmsap3.php?isdo='+datosmm[0]+'&typeisdo='+datosmm[1]+'&encont='+datosmm[2];
   });
