...
if( $_SESSION["l"]  =="NN")
{
	...
	if (isset($_POST['txtlistcustomers']))
	{
		...
		$snparamenter= $_REQUEST['snparamenter']; //
		...
		$varray_LISTUNIT = $_REQUEST['templistagainuldl']; //
			...
			$connect->beginTransaction();
				 try {
							...
							$vmaxidWO=1;
							$sql = $connect->prepare("select count(*) as cc from  orders_sn where  so_soft_external like '%".date('Ymd')."%WO%' ");
							$sql->execute();
							$resultado = $sql->fetchAll();
							foreach ($resultado as $row) {
								$vmaxidWO= $row['cc']+1;
							...
							//echo "select  * from fnt_select_allproducts_maxrev() as ppp  where modelciu in( select  classproduct from fnt_select_allproducts_maxrev() as ppp where idproduct = ".$v1.")";
							$sql = $connect->prepare("select  * from fnt_select_allproducts_maxrev() as ppp  where modelciu in( select  classproduct from fnt_select_allproducts_maxrev() as ppp where idproduct = ".$v1.")" );
							$sql->execute();
							$resultado = $sql->fetchAll();
							foreach ($resultado as $row) {
								$v_idpro_productforwo= $row['modelciu'];
								$v_idpro_idproductforwo= $row['idproduct'];
								$v1= $row['idproduct'];
								
							}	


						//	echo "*************AUTO GENERATE WO FOR LEGACY******************************<br>";
						//	echo "<br>___".$v1;
						//	echo "<br>___".$v2;
						//	echo "<br>___".date('Ymd');
							$typeregister="WO";
							$nrowoautogenrado = date('Ymd').$vmaxidWO."WO";
							
						//	echo "<br>New WO Generated:".	$nrowoautogenrado;
					//		echo "<br>New WO ModelCIU:".	$v_idpro_productforwo;
					//		echo "<br>New WO IDproduct:".	$v_idpro_idproductforwo;
					//		echo "<br>New ID ORDERS :".	$vmaxid;
						//	echo "<br>-------------------";
								
							// SET IDCURSTOMER HONEYWELL
							$v0 = 2;
							$v0 = $_REQUEST['txtlistcustomers']; ///$vidcustomer idtxtlistcustomers
							$v26=1;
							$v25="AutoCreateWOMigrate";
							...
							$sentencia->execute();
								...
								$vdescripaudit="NEW REG WO Migrate".$vuserfas;
							...
							/////////////////////////////////////////////////////////////////////////////////////
									///****** inserto orders de WO----- 1 SN
									$sentencia = $connect->prepare("INSERT INTO orders_sn(idorders, idcustomers, idfamilyprod, idtypeband, idtypeproduct, idproduct, idconfiguration, idrev, idnroserie,
									so_soft_external,wo_serialnumber, idruninfo, ponumber, pwrsupplytype, rcgfbwa, moden_dig, date_approved, descripcion,	ul_gain, ul_max_pwr, dl_gain, dl_max_pwr, 
									req_ppassy, req_calibration,	req_spec, req_other, nameapproved, notes, reqresources, typeregister, processday, processfasserver, so_original, so_associed,availablesn) VALUES (:idorders, :idcustomers, :idfamilyprod, :idtypeband, :idtypeproduct, :idproduct, :idconfiguration, :idrev, :idnroserie, :so_soft_external, :wo_serialnumber, :idruninfo, :ponumber, :pwrsupplytype, :rcgfbwa, :moden_dig, :date_approved, :descripcion, :ul_gain, :ul_max_pwr, :dl_gain, :dl_max_pwr, :req_ppassy, :req_calibration, :req_spec, :req_other, :nameapproved, :notes, :reqresources, :typeregister, :processday, :processfasserver, :so_original, :so_associed, :availablesn);");
									$vnroserie=1;
									...
									$sentencia->bindParam(':idconfiguration', $vconcero);				
									$sentencia->bindParam(':idrev', $vconcero);	
									$sentencia->bindParam(':idnroserie', $vnroserie);						
									$sentencia->bindParam(':so_soft_external', $nrowoautogenrado);
									$sentencia->bindParam(':wo_serialnumber', $snparamenter);
									$sentencia->bindParam(':idruninfo', $vconcero);					
									$sentencia->bindParam(':ponumber', $v3);					
									$sentencia->bindParam(':pwrsupplytype', $v4);
									$sentencia->bindParam(':rcgfbwa', $v5);
									...
									$sentencia->bindParam(':so_original', $vvacio);	
									$sentencia->bindParam(':so_associed', $vvacio);	
									$sentencia->bindParam(':availablesn', $vprocessdas);	
							...
							/////////////////////////////////////////////////////////////////////////////////////
								$sentenciach = $connect->prepare("insert into orders_sn_specs  SELECT idorders, idrev,   (0+ row_number() OVER ()) , 0, 'UNIT', 0, 0, 0, 0, 0, 0, v_unitdlstart, v_unitdlstop, v_unitulstart, v_unitulstop, '', idband, floor(v_ulgain), floor(v_dlgain), floor(v_ulmaxpwr), floor(v_dlmaxpwr)
									...
									$query_lista ="INSERT INTO orders_states(idorders, idstate, datestate)	VALUES (".$vmaxid.", 1, now());";
									$connect->query($query_lista);	
									$query_lista ="INSERT INTO orders_states(idorders, idstate, datestate)	VALUES (".$vmaxid.", 2, now());";
									$connect->query($query_lista);	
				...
				//	echo "<br>******  CREATE SO *************************************";
					...
					$sentencia->bindParam(':processday', $vvacionull );		
					$sentencia->bindParam(':processfasserver', $vprocessdas);	
					$sentencia->bindParam(':so_original', $nrowoautogenrado);	
					$sentencia->bindParam(':so_associed', $vvacio);	
					$sentencia->execute();
					   ...
					   foreach($porciones as $elcanaluldl) {
						...
						//	echo "dL".$separa_ULDL[0]."--uL".$separa_ULDL[1]."<br>";
							if ($elcanaluldl <> "")
							{
								...
								$sentenciach->bindParam(':idrev', $vconcero);
								$sentenciach->bindParam(':idch', $vidch);
								$sentenciach->bindParam(':idnroserie', $vnroserie);
								$sentenciach->bindParam(':typedata', $vtypedata);
								$sentenciach->bindParam(':ul_ch_fr', str_replace(",",".",$separa_ULDL[1]));
								$sentenciach->bindParam(':dl_ch_fr', str_replace(",",".",$separa_ULDL[0]));
								$sentenciach->bindParam(':dpxlowstart', $vconcero);
					...
					$sentenciach = $connect->prepare("insert into orders_sn_specs  SELECT idorders, idrev,   (0+ row_number() OVER ()) , 1, 'UNIT', 0, 0, 0, 0, 0, 0, v_unitdlstart, v_unitdlstop, v_unitulstart, v_unitulstop, '', idband, floor(v_ulgain), floor(v_dlgain), floor(v_ulmaxpwr), floor(v_dlmaxpwr)
					...
					////	  $txthavemodulerabbit
					  ...
					  $query_lista ="INSERT INTO public.orders_attributes(idorders, idattribute_orders, datemodif, v_boolean, v_integer, v_double, v_string, v_date) VALUES (".$vmaxid.", 1,now(),".$txthavemodulerabbit.", NULL, NULL, NULL, NULL);";
						...
						$query_lista ="INSERT INTO orders_states(idorders, idstate, datestate)	VALUES (".$vmaxid.", 1,   (now() - interval '3 minute') );";
						$connect->query($query_lista);

					 
							$query_lista ="INSERT INTO orders_states(idorders, idstate, datestate)	VALUES (".$vmaxid.", 2, (now() - interval '2 minute') );";
							$connect->query($query_lista);		
				 
		 
							$query_lista ="INSERT INTO orders_states(idorders, idstate, datestate)	VALUES (".$vmaxid.", 3,   (now() - interval '1 minute'));";
							$connect->query($query_lista);		
						...
						$vdescripaudit="NEW REG presales_states  -  notices_users";		
								...
								$query_lista ="INSERT INTO orders_states(idorders, idstate, datestate)	VALUES (".$vmaxid.", 7, now());";
							$connect->query($query_lista);		
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Migrate WO - SO </li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    ...
    <!-- Main content -->
		...
		<section class="col-lg-12 connectedSortable ui-sortable">
			...
			<!-- aca form -->
			      ...
			      <form action="changeuserfasdata.php" method="post" class="form-horizontal" name="frmpass" id="frmpass">
							 ...
							 <form  action="generarsaleordersmigratesowo.php" method="post" class="form-horizontal" id="myform" name="myform">
				  <!-- NUEVO RENGLON FORM  -->
				  ...
				  <div class="form-group row">
					...
					<div class="col-sm-4	">
					  <input type="text" class="form-control" id="snparamenter" name="snparamenter"  required placeholder="Serial Number"  >
					</div>
					...
					<div class="col-sm-6	">
						...
						<a href="https://webfas.honeywell.com/ticketmanagerfiplex.php" > <i class="fas fa-question-circle"></i>Require Support</a>
						</p>
						</div>
					...
					<?php   $psswdtkkey = substr( md5(microtime()), 1, 8); ?>
						...
						<div class="">
							<br>
                              <button name="btnaddatt1" id="btnaddatt1" type="button" class="btn btn-sm btn-outline-primary btn-flat " onclick="openattach(1)">click here to upload files</button>
						      <hr>
		...
		<div class="progress progress-xxs">
				...
				<div name="divluciana" id="divluciana" class="d-none">	
					...
					<label for="inputPassword" class="col-sm-1 col-form-label d-none">RC-G for BWA:</label>
					<div class="col-sm-1 d-none">
						
						<input type="checkbox"  data-toggle="toggle"  data-off="NO" data-on="YES" id="txtrcgbwa" name="txtrcgbwa" >
					</div>
					<label for="inputPassword" class="col-sm-1 col-form-label d-none">Modem for Digital:</label>
					<div class="col-sm-1 d-none">
...
<br><br>
				...
				<input type="hidden"  id="templistagainuldl" name="templistagainuldl" value="aaaaa">
					<div class="col-sm-12">
						...
						<p align="right">
						<button name="btnadddpx" id="btnadddpx" type="button" class="btn btn-sm btn-outline-primary btn-block" onclick="add_dpxdiv()">Add DPX </button>
						</p>
						...
						<div class="row">
							<table class="table table-striped table-bordered table-sm">
								...
								<select class="form-control form-control-sm" name="txtnewbanddpx" id="txtnewbanddpx" >
								<option value=""> - Select - </option>
									<?php
									...
									$sql = $connect->prepare("select * from idband where active = 'Y' order by description");
																			...
																			?>
																			<option value="<?php echo  $row2['idband']."#".$row2['fstartul']."#".$row2['fstopul']."#".$row2['fstartdl']."#".$row2['fstopdl']."#".$row2['description']; ?>">
								...
								<input type="number" min="1" class="form-control"  placeholder="000.000" name="txtnewdpxlowstart" placeholder=" " id="txtnewdpxlowstart" class="form-control" >
								</td>
								<td>
								<label>DPX Low Stop:</label>
								<input type="number" min="1" class="form-control"  placeholder="000.000" name="txtnewdpxlowstop" placeholder=" " id="txtnewdpxlowstop" class="form-control" >
								</td>
								<td>
								<label>DPX High Start:</label>
								<input  type="number" min="1" class="form-control"  placeholder="000.000" name="txtnewdpxhighstart" placeholder=" " id="txtnewdpxhighstart" class="form-control" >
								</td>
								<td>
								<label>DPX High Stop:</label>
								<input type="number" min="1" class="form-control"  placeholder="000.000" name="txtnewdpxhighstop" placeholder=" " id="txtnewdpxhighstop" class="form-control" >
								</td>
								</tr>
								</table>						
						</div>
										<button name="btnaddchannels3" id="btnaddchannels3" type="button" class="btn btn-sm btn-outline-primary btn-flat" onclick="addonedpxtolist()">Add DPX to the list </button>
						</div>
  ...
  <div class="row">
    ...
    <div class="col">	UL Channels :: copy and paste the channels here
  				...
  				<button name="btnaddchannels3" id="btnaddchannels3" type="button" class="btn btn-sm btn-outline-primary btn-flat" onclick="importar_nowl()">Import now </button>
  </div>
					...
					<div class="col-sm-10">
						...
						<button type="button" class="btn btn-primary btn-block" id="btnchangepmm" name="btnchangepmm">Migrate & Create SO</button>
					</div>
				  </div>
...
//	btnchangepmm
	...
	alert('the CIU does not have the configuration parameters for FAS');
							...
							else
							{
								...
								toastr["error"]("the CIU does not have the configuration parameters for FAS", "CIU not configured");
								$("#habilitosupport").removeClass('d-none');
...
$('#txtlistcius').select2({
 ajax: {
    url: "ajax_list_cuis_formigratewoso.php",
    dataType: 'json',
    delay: 2,
...
*/
					...
					for( var immxx = 0; immxx < tabla_dpx[i].qband; immxx++) 
					{
						...
						htmlmejoradodpx += '<tr><th width="100px" scope="row"> '+tabla_dpx[i].Band+'   &nbsp;&nbsp;   </th>';
...
//console.log(seed_associed);
  ...
  eModal.iframe('attachfileprojectso.php?idt='+seed_associed+'&idp='+vv_idp+'&idpr='+vv_idprev+'&openattach='+iddivsearch,'Attach files to SO	  ');
...
////////////////////tempporal habiamos dejado pasar a todos
	...
	{
		//alert(nropo);
		if ($("#txtsonumberlu").val().length != 10 && $("#txtsonumberlu").val().length >0)
		{
           ...
           //   console.log(data.attachlist);
              $("#myDrop"+iddivacontrolar).empty();
               ...
               //     console.log(data.attachlist[i].fileattach );
                  ...
                  //  myDrop1
                    $("#myDrop"+iddivacontrolar).append("<i class='fas fa-file'></i> "+data.attachlist[i].fileattach+" <a href='#' onclick='delattach("+ data.attachlist[i].idnroattach +" )'><i class='far fa-times-circle' style='color:red'></i></a><br>" );
                } 
