...
$elwhere="";
	...
	if ($elwhere=="")
		{
		$elwhere=" where dateinfom between current_date-100 and current_date+1 ";
		}
...
$sql = $connect->prepare("select count(*) as cc from  runinfo ");
	...
	if ($rowid ==0)
	{
	 ...
	 $sql = $connect->prepare("SELECT distinct  idruninfo, to_char(dateserver,'YYYY/MM/DD hh:mm:ss') as fechamm,  to_char(dateserver,'YYYY/MM/DD') as fechacorta,to_char(dateserver,'hh')AS hr,to_char(dateserver,'hh:mm:ss') as hrcorta,
	...
	FROM runinfo ".$elwhere." ORDER BY fechamm desc,  hr DESC, hrcorta desc  LIMIT ".$rowid." OFFSET ".$rowperpage);
    $sql->execute();
    $resultado = $sql->fetchAll();
...
$employee_arr[] = array("allcount" => $allcount);
 ...
 foreach ($resultado as $row) {
   ...
   // $idruninfo = $Encryption->encrypt($row['idruninfo'], $semillafp); // $row['idruninfo'];
    ...
    $employee_arr[] = array("idruninfo" => $idruninfo,"fechacorta" => $fechacorta,"station" => $station, "script" => $script , "device" => $device, "userruninfo" => $userruninfo, "hr" => $hr);
}
