...
?>
 ...
 if (isset($_POST['txtbusqcustom']))
 {
  ...
  select distinct fas_outcome_integral.reference ,fas_outcome_integral.datetimeref,  fas_outcome_integral.idtype , fasoutcometypename,  fas_outcome_integral.v_boolean::integer as v_booleanconvert, fas_outcome_integral2.v_string
  ...
  echo "<div class='alert alert-warning' role='alert'> (*) - Show last 3 days</div>";
      ...
      inner join (
          ...
          where idfasoutcomecat = 0  and 
                ...
                v_string = 'Connector SAP-WEBFAS' and datetimeref > now() - interval '1 day'
          ) as losrun
          on losrun.reference = fas_outcome_integral.reference
          and fas_outcome_integral.idfasoutcomecat in(0) and fas_outcome_integral.idtype=13
      ...
      on  fas_outcome_integral2.reference       = losrun.reference and
          ...
          order by fas_outcome_integral.datetimeref desc    limit 1000
      "; 
 }
...
console.log(output);
	...
	else
	{
		...
		var armando_tabla ="";
		$.ajax({
				url: 'ajax_logfassap_graphallsnrun.php?pfd='+txtfechad+'&pfh='+txtfechah ,										
				 cache:false,
