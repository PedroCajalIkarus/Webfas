								...
								<i class="expanded"><i class="fa fa-inbox"></i></i> <?php echo $row['ciu'];  ?></a> <span data-toggle="tooltip" title="" class="badge <?php echo $bgclass; ?> ">										
									...
									<?php
										for($i = 0; $i < count($lossn); $i++){
											...
											echo '<a data-ancla="anclamyTabledib" href="#" aria-expanded="true" onclick="mostrar_acceptacf(1,'."'".$row['ciu']."','".str_replace($search, $replace, $lossn[$i])."'".')">SN:'.str_replace($search, $replace, $lossn[$i]).' <i class="fas fa-eye"></i> </a>';
											echo "</span></li>";
								...
								<i class="expanded"><i class="fa fa-inbox"></i></i> <?php echo $row['ciu'];  ?></a> <span data-toggle="tooltip" title="" class="badge <?php echo $bgclass; ?> ">										
									...
									<?php
										...
										//	echo "<li value='".str_replace($search, $replace, $lossn[$i])."' id='".str_replace($search, $replace, $lossn[$i])."' class='esramahijo '>SN:".  str_replace($search, $replace, $lossn[$i])." ";
											...
											echo "<li class='treemm'><span>";
											echo '<a data-ancla="anclamyTabledib" href="#" aria-expanded="true" onclick="mostrar_acceptpa(1,'."'".$row['ciu']."','".str_replace($search, $replace, $lossn[$i])."'".')">SN:'.str_replace($search, $replace, $lossn[$i]).' <i class="fas fa-eye"></i> </a>';
											echo "</span></li>";
										}
										echo "</div></ul>";
									}
    ...
    if(returnoutput && returnoutput == true) {
		...
		//  li = document.getElementsByClassName('sui-treeview-item');
		  ...
		  for (i = 0; i < li.length; i++) {
			...
			/// Aca desaparecemos todos los item de LI
			//	$('[role="group"]').removeClass("d-none");
			/*	var output = '';
				/// filtramos x ul
				  $('[role="group"]').filter(function(index) 
				   {
						var element = $(this); 
						console.log( index + ": " + element[index].text() );
						

							console.log('**:----');
						
						
						if  (  element.text().indexOf( $("#txtacceptbusca").val()) >1)
						{
						//	console.log('el nro es :'+element.text());
						}
						else
						{
							//console.log(element.text()+ '--'+ element.attr('class'));
				  ...
				  /* $('[role="group"]').filter(function(index) 
				   {
						var element = $(this); 
				  	     for(var attr in element[0].attributes) 
						 {
                             // console.log (element[0].attributes[attr].name +'*****'+ element[0].attributes[attr].value );
                           }
					
					});*/
					
					/*$('li:contains("6092")').filter(function(index) 
				   {
						var element = $(this); 
					...
					*/
					/*$('[role="group"]:not(li:contains("6092"))').remove();
					
					
					
					$(':not(li:contains("6092"))').filter(function(index) 
				   {
						var element = $(this); 
						element.filter(":contains('treeview')").each(
						console.log (element.id +'*****'+ element.text() );
						);
					...
					$(':not(li:contains("6092"))').filter(":contains('treeview')").each( console.log(this));
					*/
